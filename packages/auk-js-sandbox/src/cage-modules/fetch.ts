  ...
  defineCageModule((ctx) => {
    ...
    const fetchFn = defineSandboxFunctionRaw(ctx, "fetch", (...args) => {
      ...
      if (init) {
        ...
        if (headersHandle) {
          ...
          const isAukHeaders = ctx.vm.getProp(headersHandle, "__isAukHeaders")
          if (isAukHeaders && ctx.vm.typeof(isAukHeaders) === "boolean") {
            const isAukHeadersValue = ctx.vm.dump(isAukHeaders)
            if (isAukHeadersValue === true) {
    ...
    const createHeadersInstance = defineSandboxFunctionRaw(
      ...
      (initHandle) => {
        ...
        ctx.vm.setProp(headersInstance, "__isAukHeaders", ctx.vm.true)
