...
describe("auk.env.active.get", () => {
  test("returns the value from selected environment if present", () => {
    return expect(
      runTest(
        `
          const data = auk.env.active.get("a")
          auk.expect(data).toBe("selectedVal")
        `,
        {
          selected: [
            {
  ...
  test("returns null if key does not exist in selected", () => {
    return expect(
      runTest(
        `
          const data = auk.env.active.get("absent")
          auk.expect(data).toBe(null)
        `,
        {
          selected: [],
...
describe("auk.env.global.get", () => {
  test("returns the value from global environment if present", () => {
    return expect(
      runTest(
        `
          const data = auk.env.global.get("foo")
          auk.expect(data).toBe("globalVal")
        `,
        {
          selected: [
            {
  ...
  test("returns null if key does not exist in global", () => {
    return expect(
      runTest(
        `
          const data = auk.env.global.get("not_here")
          auk.expect(data).toBe(null)
        `,
        {
          selected: [
