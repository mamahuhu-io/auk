...
type BodyReturnType =
  ...
  | {
      type: "NON_FORMDATA"
      body: Exclude<AukRESTReqBody, AukRESTReqBodyFormData>
    }
...
const getBodyFromContentType =
  (rData: string, cType: AukRESTReqBody["contentType"]) => (rct: string) =>
    pipe(
      cType,
...
const getContentTypeFromRawContentType = (rawContentType: string) =>
  pipe(
    ...
    O.map((ct) => ct as AukRESTReqBody["contentType"])
  )
...
export const getBody = (
  ...
  contentType: AukRESTReqBody["contentType"]
