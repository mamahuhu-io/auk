...
;(inputs) => {
  ...
  globalThis.auk = {
    ...
    // Expose fetch as auk.fetch() for explicit access
    // Note: This exposes the fetch implementation provided by the host environment via aukFetchHook
  ...
  globalThis.pm = {
    environment: {
      get: (key) => {
        const value = globalThis.auk.env.active.get(key)
      ...
      unset: (key) => globalThis.auk.env.active.delete(key),
      ...
      toObject: () => {
        ...
        const result = {}
        envVars.forEach((envVar) => {
          const value = globalThis.auk.env.active.get(envVar.key)
          if (value !== null) {
    ...
    globals: {
      get: (key) => {
        const value = globalThis.auk.env.global.get(key)
        // Postman returns undefined for missing keys, not null
        return value === null ? undefined : value
      },
      set: (key, value) => {
      ...
      unset: (key) => globalThis.auk.env.global.delete(key),
      ...
      toObject: () => {
        ...
        const result = {}
        envVars.forEach((envVar) => {
          const value = globalThis.auk.env.global.get(envVar.key)
          if (value !== null) {
            result[envVar.key] = value
          }
        })
    ...
    variables: {
      get: (key) => {
        const value = globalThis.auk.env.get(key)
        // Postman returns undefined for missing keys, not null
        return value === null ? undefined : value
      },
      set: (key, value) => {
    ...
    request: {
      ...
      get url() {
        ...
        const urlObj = {
          ...
          _parseUrl: () => {
            ...
            try {
              ...
              // Only merge from auk.request.params if URL has no query params
              // This supports imported collections while allowing mutations to work
              let finalParams = urlParams
              if (urlParams.length === 0) {
                // No params in URL - check auk.request.params (for imported collections)
                const requestParams = globalThis.auk.request.params || []
              ...
              // Fallback: try to get params from auk.request.params
              const requestParams = globalThis.auk.request?.params || []
          ...
          update: (urlString) => {
            if (typeof urlString === "string") {
              globalThis.auk.request.setUrl(urlString)
            } else if (
              urlString &&
              typeof urlString === "object" &&
              typeof urlString.toString === "function"
            ) {
              globalThis.auk.request.setUrl(urlString.toString())
            } else {
              throw new Error(
          ...
          addQueryParams: (params) => {
            ...
            globalThis.auk.request.setUrl(urlObj._rebuildUrl(currentParsed))
          },
          ...
          removeQueryParams: (params) => {
            ...
            globalThis.auk.request.setParams(
              updatedParams.map((p) => ({
                key: p.key,
                value: p.value,
                active: true,
        ...
        Object.defineProperty(urlObj, "protocol", {
          ...
          set: (value) => {
            ...
            globalThis.auk.request.setUrl(urlObj._rebuildUrl(parsed))
          },
          enumerable: true,
        })
        ...
        Object.defineProperty(urlObj, "hash", {
          ...
          set: (value) => {
            const current = globalThis.auk.request.url
            ...
            const hashValue = value
              ? value.startsWith("#")
                ...
                : `#${value}`
              : ""
            globalThis.auk.request.setUrl(baseUrl + hashValue)
          },
          enumerable: true,
        })
      ...
      set url(value) {
        ...
        try {
          ...
          globalThis.auk.request.setParams(urlParams)
        } catch {
      ...
      set method(value) {
        ...
        globalThis.auk.request.setMethod(value)
      },
      ...
      get headers() {
        return {
          ...
          add: (header) => {
            ...
            globalThis.auk.request.setHeader(header.key, header.value || "")
          },
          ...
          remove: (headerName) => {
            ...
            globalThis.auk.request.removeHeader(headerName)
          },
          ...
          upsert: (header) => {
            ...
            globalThis.auk.request.removeHeader(header.key)
            globalThis.auk.request.setHeader(header.key, header.value || "")
          },

          clear: () => {
            globalThis.auk.request.setHeaders([])
          },
          ...
          insert: (item, before) => {
            ...
            if (before) {
              ...
              if (beforeIdx >= 0) {
                ...
                globalThis.auk.request.setHeaders(newHeaders)
              } else {
                // If 'before' not found, append to end
                globalThis.auk.request.setHeader(item.key, item.value || "")
              }
            } else {
              // No 'before' specified, add to end
              globalThis.auk.request.setHeader(item.key, item.value || "")
          ...
          append: (item) => {
            ...
            globalThis.auk.request.removeHeader(item.key)
          ...
          assimilate: (source, prune) => {
            ...
            if (prune) {
              ...
              const currentHeaders = globalThis.auk.request.headers
              const filteredHeaders = currentHeaders.filter((h) =>
                sourceKeys.includes(h.key.toLowerCase())
              )
              globalThis.auk.request.setHeaders(filteredHeaders)
            }
          },
        }
      },
      ...
      get body() {
        const currentBody = globalThis.auk.request.body
        ...
        return {
          ...
          update: (bodySpec) => {
            if (typeof bodySpec === "string") {
              ...
              globalThis.auk.request.setBody({
      ...
      set auth(value) {
        if (value === null || value === undefined) {
          globalThis.auk.request.setAuth({
            authType: "none",
            authActive: false,
          })
        } else if (typeof value === "object") {
          globalThis.auk.request.setAuth(value)
        } else {
    ...
    sendRequest: (urlOrRequest, callback) => {
      ...
      // Call auk.fetch() and adapt response
      globalThis.auk
        .fetch(url, options)
        .then(async (response) => {
