...
// HACK: To solve the AxiosRequestConfig being different between @auk/common
...
export class AgentInterceptorService extends Service implements Interceptor {
  ...
  public settingsPageEntry = {
    entryTitle: (t: ReturnType<typeof getI18n>) =>
          ...
          const response = await axios.get(
            "http://localhost:9119/registered-handshake",
            {
              headers: {
                ...
                "X-Auk-Nonce": nonceB16,
              },
              responseType: "arraybuffer",
            }
          )

          const responseNonceB16: string = response.headers["x-auk-nonce"]
  ...
  public async performHandshake(): Promise<void> {
    ...
    if (
      ...
      handshakeResponse.data.__auk__agent__ === true
    ) {
    ...
    return {
      ...
      response: (async () => {
        ...
        try {
          ...
          const responseNonceB16: string = http_response.headers["x-auk-nonce"]
