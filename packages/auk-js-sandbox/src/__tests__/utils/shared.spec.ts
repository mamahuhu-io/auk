...
describe("getSharedEnvMethods - Experimental Sandbox (isAukNamespace=true)", () => {
  ...
  test("auk.set creates new variable in selected scope (default source='all')", () => {
    const { methods, updatedEnvs } = getSharedEnvMethods(baseEnvs, true)

    methods.auk.set("aukKey", "aukVal")
  ...
  test("auk.set validates string types", () => {
    const { methods } = getSharedEnvMethods(baseEnvs, true)

    expect(() => methods.auk.set(123 as any, "value")).toThrow(
      "Expected key to be a string"
    )
    expect(() => methods.auk.set("key", 123 as any)).toThrow(
      "Expected value to be a string"
    )
  })

  test("auk.delete removes variable from selected scope", () => {
    const { methods, updatedEnvs } = getSharedEnvMethods(baseEnvs, true)

    methods.auk.delete("selectedKey")
  ...
  test("auk.reset resets a variable to its initial value", () => {
  ...
  test("auk.getInitialRaw returns initial value", () => {
    const { methods } = getSharedEnvMethods(
      {
        global: [],
        selected: [
    ...
    expect(methods.auk.getInitialRaw("testKey")).toBe("initialVal")
    expect(methods.auk.getInitialRaw("nonexistent")).toBeNull()
  })

  test("auk.setInitial sets initial value", () => {
    const { methods, updatedEnvs } = getSharedEnvMethods(baseEnvs, true)

    methods.auk.setInitial("initKey", "initVal")
...
describe("getSharedEnvMethods - Legacy Sandbox (isAukNamespace=false)", () => {
  const baseEnvs: TestResult["envs"] = {
    global: [
      {
        key: "globalKey",
