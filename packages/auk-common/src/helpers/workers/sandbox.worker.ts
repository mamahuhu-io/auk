import { Cookie, Environment, AukRESTRequest } from "@auk/data"
import {
...
self.addEventListener(
  ...
  async (event: MessageEvent<IncomingSandboxWorkerMessage>) => {
    ...
    const parsedRequest = JSON.parse(request) as AukRESTRequest

    const parsedRequestResult = AukRESTRequest.safeParse(parsedRequest)
