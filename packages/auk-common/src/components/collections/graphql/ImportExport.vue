...
import { aukGqlCollectionsImporter } from "~/helpers/import-export/import/aukGql"
import { gqlCollectionsExporter } from "~/helpers/import-export/export/gqlCollections"
import { gistExporter } from "~/helpers/import-export/export/gist"
import { computed } from "vue"
import { aukGQLImporter } from "~/helpers/import-export/import/auk"
...
const GqlCollectionsAukImporter: ImporterOrExporter = {
  metadata: {
    id: "import.from_json",
  ...
  component: FileSource({
    ...
    onImportFromFile: async (gqlCollections) => {
      const res = await aukGqlCollectionsImporter(gqlCollections)

      if (E.isLeft(res)) {
        showImportFailedError()
        return
      }
      const validatedCollection = await aukGQLImporter(
...
const GqlCollectionsGistImporter: ImporterOrExporter = {
  ...
  component: GistSource({
    ...
    onImportFromGist: async (gqlCollections) => {
      ...
      const res = await aukGqlCollectionsImporter(gqlCollections.right)
...
const GqlCollectionsAukExporter: ImporterOrExporter = {
  metadata: {
    id: "export.as_json",
    name: "export.as_json",
...
const importerModules = [GqlCollectionsAukImporter, GqlCollectionsGistImporter]

const exporterModules = computed(() => {
  const modules = [GqlCollectionsAukExporter]
...
const handleImportToStore = (gqlCollections: AukCollection[]) => {
