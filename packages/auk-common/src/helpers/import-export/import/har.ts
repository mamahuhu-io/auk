...
export const harImporter = (
  content: string[]
): E.Either<"INVALID_HAR" | "SOMETHING_WENT_WRONG", AukCollection[]> => {
  try {
    ...
    const requests = harToAukRequestConverter(har)
...
const harToAukRequestConverter = (har: HAR) => {
  return har.log.entries.map((entry): AukRESTRequest => {
    const headers = entry.request.headers.map((header): AukRESTHeader => {
      return {
        active: true,
        key: header.name,
        value: header.value,
    ...
    const params = entry.request.queryString.map((param): AukRESTParam => {
      return {
        active: true,
        key: param.name,
        value: param.value,
    ...
    const body = entry.request.postData
      ? convertPostDataToAukBody(entry.request.postData)
...
const convertPostDataToAukBody = (
  ...
  if (contentType === "application/x-www-form-urlencoded") {
    ...
    if (postData.text) {
      bodyContent = formatXWWWFormUrlencodedForAuk(postData.text)
  ...
  if (contentType === "multipart/form-data") {
    const body: AukRESTReqBodyFormData = {
      contentType: "multipart/form-data",
      body:
...
const formatXWWWFormUrlencodedForAuk = (text: string) => {
