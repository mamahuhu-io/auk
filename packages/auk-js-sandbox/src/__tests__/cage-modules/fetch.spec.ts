...
const hookWithHeaders: AukFetchHook = async () => {
...
const hookNoHeaders: AukFetchHook = async () => {
  const resp = new Response(jsonText, {
    status: 200,
...
const runCage = async (script: string, hook: AukFetchHook) => {
  const cage = await FaradayCage.create()
  const result = await cage.runCode(script, [
    ...defaultModules({ aukFetchHook: hook }),
  ])
  return result
}
...
describe("fetch cage module", () => {
  ...
  it("basic fetch works and calls hook", async () => {
    ...
    const capturingHook: AukFetchHook = async (input, init) => {
  ...
  it("fallback builds _bodyBytes when hook returns native Response", async () => {
    const nativeHook: AukFetchHook = async () =>
      new Response("Zed", {
        status: 200,
  ...
  it("fetch with aborted signal rejects (message-based)", async () => {
    const abortAwareHook: AukFetchHook = async (_input, init) => {
  ...
  it("network errors propagate as FetchError with message", async () => {
    const failingHook: AukFetchHook = async () => {
