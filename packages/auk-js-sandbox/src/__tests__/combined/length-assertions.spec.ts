...
const NAMESPACES = ["pm", "auk"] as const
    ...
    test("should pass when array length meets minimum (.at.least)", async () => {
      const script = `
        ${namespace}.test("Array length at least", () => {
          const items = ${namespace === "pm" ? "pm.response.json()" : "auk.response.body.asJSON()"}.items
    ...
    test("should work with empty arrays", async () => {
      const script = `
        ${namespace}.test("Empty array", () => {
          const empty = ${namespace === "pm" ? "pm.response.json()" : "auk.response.body.asJSON()"}.emptyArray
    ...
    test("should work with nested arrays", async () => {
      const script = `
        ${namespace}.test("Nested array length", () => {
          const nested = ${namespace === "pm" ? "pm.response.json()" : "auk.response.body.asJSON()"}.data.nested.values
