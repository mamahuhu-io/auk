import { defineVersion, entityRefUptoVersion } from "verzod"
import { z } from "zod"

import { GQLHeader as V1_GQLHeader } from "../../graphql/v/1"
import { AukRESTHeaders as V1_AukRESTHeaders } from "../../rest/v/1"

import { AukGQLAuth, GQLHeader as V2_GQLHeader } from "../../graphql/v/6"
import {
  AukRESTAuth,
  AukRESTHeaders as V2_AukRESTHeaders,
} from "../../rest/v/7"

import { v2_baseCollectionSchema, V2_SCHEMA } from "./2"
import { AukCollection } from ".."

export const v3_baseCollectionSchema = v2_baseCollectionSchema.extend({
  v: z.literal(3),

  // AWS Signature Authorization type addition
  auth: z.union([AukRESTAuth, AukGQLAuth]),

  // `description` field addition under `headers`
  headers: z.union([V2_AukRESTHeaders, z.array(V2_GQLHeader)]),
})

type Input = z.input<typeof v3_baseCollectionSchema> & {
  folders: Input[]
}

type Output = z.output<typeof v3_baseCollectionSchema> & {
  folders: Output[]
}

export const V3_SCHEMA = v3_baseCollectionSchema.extend({
  folders: z.lazy(() => z.array(entityRefUptoVersion(AukCollection, 3))),
}) as z.ZodType<Output, z.ZodTypeDef, Input>

export default defineVersion({
  initial: false,
  schema: V3_SCHEMA,
  up(old: z.infer<typeof V2_SCHEMA>) {
    const headers = (old.headers as V1_AukRESTHeaders | V1_GQLHeader[]).map(
      (header) => ({
        ...header,
        description: "",
      })
    )

    const result: z.infer<typeof V3_SCHEMA> = {
      ...old,
      v: 3,
      headers,
      folders: old.folders.map((folder) => {
        const result = AukCollection.safeParseUpToVersion(folder, 3)

        if (result.type !== "ok") {
          throw new Error("Failed to migrate child collections")
        }

        return result.value
      }),
    }

    return result
  },
})
