...
import { AukErrorCode } from "../../../types/errors";
import {
  getErrorCode,
  getTestJsonFilePath,
  runCLI,
...
const hasTransientExternalFailure = (output: string) =>
  /ECONNRESET|EAI_AGAIN|ENOTFOUND|ETIMEDOUT|ECONNREFUSED|EPERM/i.test(
    output
  ) ||
  (/REQUEST_ERROR/i.test(output) && !/Invalid URL/i.test(output)) ||
...
describe("auk test [options] <file_path_or_id>", { timeout: 100000 }, () => {
  const VALID_TEST_ARGS = `test ${getTestJsonFilePath("passes-coll.json", "collection")}`;

  describe("Test `auk test <file_path_or_id>` command:", () => {
    describe("Argument parsing", () => {
      test("Errors with the code `INVALID_ARGUMENT` for not supplying enough arguments", async () => {
        ...
        expect(out).toBe<AukErrorCode>("INVALID_ARGUMENT");
      });
    ...
    describe("Supplied collection export file validations", () => {
      test("Errors with the code `FILE_NOT_FOUND` if the supplied collection export file doesn't exist", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("FILE_NOT_FOUND");
      });
      ...
      test("Errors with the code UNKNOWN_ERROR if the supplied collection export file content isn't valid JSON", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("UNKNOWN_ERROR");
      });
      ...
      test("Errors with the code `MALFORMED_COLLECTION` if the supplied collection export file content is malformed", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("MALFORMED_COLLECTION");
      });
      ...
      test("Errors with the code `INVALID_FILE_TYPE` if the supplied collection export file doesn't end with the `.json` extension", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("INVALID_FILE_TYPE");
      });
     ...
     */
    test("Supports the new scripting API method additions under the `auk` and `pm` namespaces and validates JUnit report structure", async () => {
      ...
      expect(junitXml).toContain('name="auk.expect');

      // 8. auk.fetch() and pm.sendRequest() tests
      expect(junitXml).toContain(
        'name="auk.fetch() should make successful GET request'
      );
      expect(junitXml).toContain(
        'name="pm.sendRequest() should work with string URL'
      );
      expect(junitXml).toContain(
        'name="auk.fetch() should handle binary responses'
      );
  ...
  describe("Test `auk test <file_path_or_id> --env <file_path_or_id>` command:", () => {
    describe("Supplied environment export file validations", () => {
      ...
      test("Errors with the code `MALFORMED_ENV_FILE` on supplying a malformed environment export file", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("MALFORMED_ENV_FILE");
      });
      ...
      test("Errors with the code `BULK_ENV_FILE` on supplying an environment export file based on the bulk environment export format", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("BULK_ENV_FILE");
      });
    });
  ...
  describe("Test `auk test <file_path_or_id> --delay <delay_in_ms>` command:", () => {
    describe("Argument parsing", () => {
  ...
  describe("Test`auk test <file_path_or_id> --env <file_path_or_id> --reporter-junit [path]", () => {
    const genPath = path.resolve("auk-cli-test");
    ...
    test("Report export fails with the code `REPORT_EXPORT_FAILED` while encountering an error during path creation", async () => {
      const exportPath = "auk-junit-report.xml";
      ...
      const { stdout, stderr } = await runCLI(args, {
        cwd: path.resolve("auk-cli-test"),
      });

      const combinedOutput = `${stdout}\n${stderr}`;
      const hasNetworkError = hasTransientExternalFailure(combinedOutput);

      if (hasNetworkError) {
        console.warn(
          "⚠️  Skipping report export error test due to transient network failure."
        );
        return;
      }

      const out = getErrorCode(stderr);
      expect(out).toBe<AukErrorCode>("REPORT_EXPORT_FAILED");
    ...
    test("Generates a JUnit report at the default path", async () => {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        const combinedArtifacts = `${fileContents}\n${lastResult.stdout}\n${lastResult.stderr}`;
        const hasNetworkErrorInXML =
          hasTransientExternalFailure(combinedArtifacts);
    ...
    test("Generates a JUnit report for a collection referring to environment variables", async () => {
      ...
      const normalizedJunitXml = replaceDynamicValuesInStr(lastFileContents);

      // Avoid snapshot coupling here: this test is network-sensitive and can
      // intermittently skip, which can leave stale snapshots in CI.
      expect(normalizedJunitXml).toContain(
        '<testsuites tests="12" failures="0" errors="0"'
      );
      expect(normalizedJunitXml).toContain(
        'name="Test environment variables in request body/test-request"'
      );
      expect((normalizedJunitXml.match(/<testcase /g) || []).length).toBe(12);
      expect(normalizedJunitXml).toContain(
        "Successfully resolves environments recursively"
      );
      expect(normalizedJunitXml).toContain(
        "Successfully resolves environments referenced in the request body"
      );
      expect(normalizedJunitXml).not.toContain("<failure ");
      expect(normalizedJunitXml).not.toContain("<error ");
    });
  });

  describe("Test `auk test <file> --iteration-count <no_of_iterations>` command:", () => {
    const VALID_TEST_ARGS = `test ${getTestJsonFilePath("passes-coll.json", "collection")}`;
  ...
  describe("Test `auk test <file> --iteration-data <file_path>` command:", () => {
    describe("Supplied data export file validations", () => {
      ...
      test("Errors with the code `INVALID_DATA_FILE_TYPE` if the supplied data file doesn't end with the `.csv` extension", async () => {
        ...
        const out = getErrorCode(stderr);
        expect(out).toBe<AukErrorCode>("INVALID_DATA_FILE_TYPE");
      });
