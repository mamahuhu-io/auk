 ...
 * ensuring env mutations from async callbacks (like auk.fetch().then()) are included.
 */
function registerAfterScriptExecutionHook(
  _ctx: CageModuleCtx,
  _type: ModuleType,
...
function createScriptingInputsObj(
  ...
  type: "pre",
  config: PreRequestModuleConfig,
  captureGetUpdatedRequest?: (fn: () => AukRESTRequest) => void
): PreRequestInputs
function createScriptingInputsObj(
  ctx: CageModuleCtx,
  type: "post",
...
const createScriptingModule = (
  ...
  return defineCageModule((ctx) => {
    ...
    let getUpdatedRequest: (() => AukRESTRequest) | undefined = undefined
