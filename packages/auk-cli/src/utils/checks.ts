...
import { AukCLIError, AukErrnoException } from "../types/errors";
...
/**
 * Checks if given error data is of type AukCLIError, based on existence
 * of code property.
 * @param error Error data to check.
 * @returns True, if unknown error validates to be AukCLIError;
 * False, otherwise.
 */
export const isAukCLIError = (error: unknown): error is AukCLIError => {
  return (
    !!error &&
    typeof error === "object" &&
...
/**
 * Checks if given error data is of type AukErrnoException, based on existence
 * of name property.
 * @param error Error data to check.
 * @returns True, if unknown error validates to be AukErrnoException;
 * False, otherwise.
 */
export const isAukErrnoException = (
  error: unknown
): error is AukErrnoException => {
  return (
    !!error &&
    typeof error === "object" &&
