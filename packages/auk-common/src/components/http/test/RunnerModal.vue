<template>
  ...
  >
    <template #body>
      <AukSmartTabs v-model="activeTab">
        <AukSmartTab id="gui" :label="t('collection_runner.ui')">
          <div
              ...
              <div class="flex flex-col gap-4 mt-4 items-start">
                ...
                >
                  ...
                  <AukButtonSecondary
                    ...
                    class="!py-0 pl-2"
                    to="https://auk.mamahuhu.dev/documentation/features/inspections"
                    blank
                    :title="t('app.wiki')"
                    :icon="IconHelpCircle"
                  />
        ...
        <AukSmartTab id="cli" :label="t('collection_runner.cli')">
...
import { getRESTCollectionByRefId } from "~/store/collections"
...
const runnerLink = computed(() => {
  return activeTab.value === "gui"
    ? "https://auk.mamahuhu.dev/documentation/features/runner#runner"
    : "https://auk.mamahuhu.dev/documentation/clients/cli/overview#running-collections-present-on-the-api-client"
})
...
const runTests = async () => {
  ...
  tabs.createNewTab({
    ...
    collection: collectionTree as AukCollection,
    isDirty: false,
    config: config.value,
    status: "idle",
    request: null,
...
/**
 * Fetches the collection tree from local store
 * @param collection
...
const CLICommand = computed(() => {
  ...
  const environmentFlag =
    includeEnvironmentID.value && environmentID.value
      ? "-e <environment_file_path.json>"
      : ""

  return `auk test <collection_file_path.json> ${environmentFlag}`.trim()
})
