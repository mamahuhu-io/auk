...
/**
 * Comprehensive tests for auk.fetch() and global fetch() API
 *
 ...
 *
 * The actual network requests are mocked via the aukFetchHook parameter.
 */

describe("auk.fetch() and global fetch()", () => {
  describe("Basic functionality", () => {
    test("auk.fetch should be defined and callable", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            pw.expect(typeof auk.fetch).toBe("function")
          `,
          { global: [], selected: [] },
          undefined,
          undefined,
    ...
    test("auk.fetch should make GET request with string URL", async () => {
      const mockFetch: AukFetchHook = vi.fn(async (_input, _init) => {
      ...
      await expect(
        runTest(
          `
            const response = await auk.fetch("https://api.example.com/data")
    ...
    test("auk.fetch should make POST request with JSON body", async () => {
      const mockFetch: AukFetchHook = vi.fn(async (_input, _init) => {
      ...
      await expect(
        runTest(
          `
            const response = await auk.fetch("https://api.example.com/items", {
              method: "POST",
              headers: {
    ...
    test("auk.fetch should handle URL object", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
        return new Response("OK", { status: 200 })
      })
      ...
      await expect(
        runTest(
          `
            ...
            const response = await auk.fetch(url)
            pw.expect(response.status).toBe(200)
          `,
          { global: [], selected: [] },
          undefined,
  ...
  describe("Response handling", () => {
    test("auk.fetch should handle text response", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            const response = await auk.fetch("https://api.example.com/text")
            pw.expect(response.status).toBe(200)
            pw.expect(response.ok).toBe(true)
          `,
          { global: [], selected: [] },
    ...
    test("auk.fetch should handle response headers", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
    ...
    test("auk.fetch should handle status codes", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            const response = await auk.fetch("https://api.example.com/missing")
  ...
  describe("HTTP methods", () => {
    test("auk.fetch should support PUT method", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            const response = await auk.fetch("https://api.example.com/items/1", {
              method: "PUT",
    ...
    test("auk.fetch should support DELETE method", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
    ...
    test("auk.fetch should support PATCH method", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
  ...
  describe("Headers", () => {
    test("auk.fetch should send custom headers", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
        return new Response("OK", { status: 200 })
      })
  ...
  describe("Error handling", () => {
    test("auk.fetch should handle fetch errors", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            ...
            try {
              await auk.fetch("https://api.example.com/data")
            } catch (error) {
              errorOccurred = true
  ...
  describe("Integration with environment variables", () => {
    test("auk.fetch should work with dynamic URLs from env vars", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            auk.env.set("API_URL", "https://api.example.com")
            const url = auk.env.get("API_URL") + "/data"
            const response = await auk.fetch(url)
            pw.expect(response.status).toBe(200)
          `,
          { global: [], selected: [] },
    ...
    test("auk.fetch should store response data in env vars", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
      ...
      await expect(
        runTest(
          `
            const response = await auk.fetch("https://api.example.com/auth")
            pw.expect(response.status).toBe(200)
            auk.env.set("AUTH_TOKEN", "abc123")
            pw.expect(auk.env.get("AUTH_TOKEN")).toBe("abc123")
          `,
          { global: [], selected: [] },
          undefined,
          undefined,
  ...
  describe("Global fetch() alias", () => {
    ...
    test("global fetch() should work identically to auk.fetch()", async () => {
      const mockFetch: AukFetchHook = vi.fn(async (_input, _init) => {
        return new Response(JSON.stringify({ success: true }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
    ...
    test("global fetch() and auk.fetch() should call the same hook", async () => {
      const mockFetch: AukFetchHook = vi.fn(async () => {
        return new Response("OK", { status: 200 })
      })
      ...
      await expect(
        runTest(
          `
            ...
            await auk.fetch("https://api.example.com/test2")
            pw.expect(1).toBe(1)
          `,
          { global: [], selected: [] },
          undefined,
  ...
  describe("Body methods", () => {
    test("response.arrayBuffer() returns array of bytes", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("arrayBuffer returns array", async () => {
              const response = await auk.fetch("https://api.example.com/binary")
    ...
    test("response.blob() returns blob object with size and type", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("blob returns blob object", async () => {
              const response = await auk.fetch("https://api.example.com/data")
    ...
    test("response.formData() parses form-encoded data", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("formData parses data", async () => {
              const response = await auk.fetch("https://api.example.com/form")
  ...
  describe("Body consumption tracking", () => {
    test("bodyUsed should be false initially and true after consuming", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("bodyUsed tracks consumption", async () => {
              const response = await auk.fetch("https://api.example.com/data")
    ...
    test("reading body twice should throw error", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("cannot read body twice", async () => {
              const response = await auk.fetch("https://api.example.com/data")
    ...
    test("different body methods should all consume the body", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("arrayBuffer consumes body", async () => {
              const response = await auk.fetch("https://api.example.com/data")
  ...
  describe("Response cloning", () => {
    test("response.clone() creates independent copy with separate body consumption", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("clone has independent body", async () => {
              try {
                const response = await auk.fetch("https://api.example.com/data")
                pw.expect(typeof response.clone).toBe("function")
    ...
    test("cloned response should preserve all properties and headers", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("clone preserves properties", async () => {
              try {
                const response = await auk.fetch("https://api.example.com/create")
                const clone = response.clone()
    ...
    test("cloning consumed response should fail", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("cannot clone consumed response", async () => {
              const response = await auk.fetch("https://api.example.com/data")
      ...
      await expect(
        runTest(
          `
            ...
            const response = await auk.fetch("https://api.example.com/data", { headers })
            pw.expect(response.status).toBe(200)
          `,
          { global: [], selected: [] },
          undefined,
  ...
  describe("Edge cases", () => {
    ...
    test("empty response body should be handled correctly", async () => {
      ...
      await expect(
        runTest(
          `
            auk.test("empty body handled", async () => {
              const response = await auk.fetch("https://api.example.com/delete")
              pw.expect(response.status).toBe(204)
