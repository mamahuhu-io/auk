...
const buildHarHeaders = (req: AukRESTRequest): Har.Header[] => {
  return req.headers
...
const buildHarQueryStrings = (req: AukRESTRequest): Har.QueryString[] => {
  return req.params
...
const buildHarPostParams = (
  req: AukRESTRequest &
    FieldEquals<AukRESTRequest, "method", ["POST", "PUT"]> & {
      body: {
...
const buildHarPostData = (req: AukRESTRequest): Har.PostData | undefined => {
...
export const buildHarRequest = (
  ...
  return {
    ...
    cookies: [], // Auk does not have formal support for Cookies as of right now
