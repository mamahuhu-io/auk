import { AukRESTRequest, AukRESTAuthOAuth2 } from "@auk/data"
import { AuthType } from "@auk/kernel"
import * as E from "fp-ts/Either"
import * as O from "fp-ts/Option"
import * as TE from "fp-ts/TaskEither"
import { flow, pipe } from "fp-ts/function"

type AukAuth = AukRESTRequest["auth"]
type OAuth2GrantType = AukRESTAuthOAuth2["grantTypeInfo"]
type GrantType = Extract<AuthType, { kind: "oauth2" }>["grantType"]

export const defaultAuth: AuthType = { kind: "none" }

const isAuthActive = (auth: AukAuth): boolean =>
  auth.authActive && auth.authType !== "none" && auth.authType !== "inherit"

const Guards = {
  basic: flow(
    O.fromPredicate(
      (auth: AukAuth): auth is AukAuth & { authType: "basic" } =>
        auth.authType === "basic"
    )
  ),
  bearer: flow(
    O.fromPredicate(
      (auth: AukAuth): auth is AukAuth & { authType: "bearer" } =>
        auth.authType === "bearer"
    )
  ),
  apiKey: flow(
    O.fromPredicate(
      (auth: AukAuth): auth is AukAuth & { authType: "api-key" } =>
        auth.authType === "api-key"
    )
  ),
  aws: flow(
    O.fromPredicate(
      (auth: AukAuth): auth is AukAuth & { authType: "aws-signature" } =>
        auth.authType === "aws-signature"
    )
  ),
  digest: flow(
    O.fromPredicate(
      (auth: AukAuth): auth is AukAuth & { authType: "digest" } =>
        auth.authType === "digest"
    )
  ),
  oauth2: flow(
    O.fromPredicate(
      (auth: AukAuth): auth is AukAuth & { authType: "oauth-2" } =>
...
type AuthProcessor<T extends AukAuth = AukAuth> = (
  auth: T
...
const getProcessor = (
  auth: AukAuth
): O.Option<(auth: AukAuth) => E.Either<Error, AuthType>> =>
  pipe(
...
export const transformAuth = (auth: AukAuth): TE.TaskEither<Error, AuthType> =>
  pipe(
    auth,
