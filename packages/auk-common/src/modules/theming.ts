...
import type { AukBgColor } from "~/store/settings"
import { PersistenceService } from "~/services/persistence"
import { AukModule } from "."
import { getService } from "./dioc"

export type AukColorMode = {
  preference: AukBgColor
  value: Readonly<Exclude<AukBgColor, "system">>
}
...
const applyColorMode = (app: App) => {
  ...
  const currentLocalPreference = useStorage<AukBgColor>(
    "nuxt-color-mode",
    "system",
    persistenceService.localConfigStorage,
    {
  ...
  const selection = computed<Exclude<AukBgColor, "system">>(() => {
  ...
  const exposed: AukColorMode = reactive({
    preference: currentLocalPreference,
    // Marking as readonly to not allow writes to this ref
    value: selection as Readonly<Ref<Exclude<AukBgColor, "system">>>,
  })
