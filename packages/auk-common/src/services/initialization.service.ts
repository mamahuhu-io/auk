 ...
 * Simplified for local-only mode - no backend client initialization.
 */
export class InitializationService extends Service<InitEvent> {
  ...
  private get interceptorService() {
    return this.bind(KernelInterceptorService)
  }
  private get nativeInterceptorService() {
    return this.bind(NativeKernelInterceptorService)
  }
  ...
  private async initStore() {
    const result = await this.persistenceService.init()
  ...
  private async initPersistenceFirst() {
    ...
    await this.persistenceService.setupFirst()
  ...
  private async initTabs() {
    ...
    await Promise.all([this.restTabService.init(), this.gqlTabService.init()])
  ...
  private async initNativeKernelNetworking() {
    this.interceptorService.register(this.nativeInterceptorService)
    this.interceptorService.setActive("native")
  ...
  private async initPersistenceLater() {
    ...
    await this.persistenceService.setupLater()
