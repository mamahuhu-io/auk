...
import { StoreError } from "@auk/kernel"
...
import { WorkspaceEnvironmentsService } from "../workspace-environments.service"
...
import { bulkApplyLocalState, localStateStore } from "../../store/localstate"
...
import { MQTTRequest$, setMQTTRequest } from "../../store/MQTTSession"
import { SSERequest$, setSSERequest } from "../../store/SSESession"
import { SIORequest$, setSIORequest } from "../../store/SocketIOSession"
import { WSRequest$, setWSRequest } from "../../store/WebSocketSession"
...
export class PersistenceService extends Service {
  ...
  public localConfigStorage: StorageLike = localStorage

  // Use lazy binding to avoid circular dependency
  private get restTabService() {
    return this.bind(RESTTabService)
  }
  private get gqlTabService() {
    return this.bind(GQLTabService)
  }
  private get secretEnvironmentService() {
    return this.bind(SecretEnvironmentService)
  }
  private get currentEnvironmentValueService() {
    return this.bind(CurrentValueService)
  }
  private get currentSortValuesService() {
    return this.bind(CurrentSortValuesService)
  }
  ...
  private async checkAndMigrateOldSettings() {
    ...
    if (themeColor) {
      ...
      if (result.success) {
        applySetting("THEME_COLOR", result.data as AukAccentColor)
      } else {
    ...
    if (nuxtColorMode) {
      ...
      if (result.success) {
        applySetting("BG_COLOR", result.data as AukBgColor)
      } else {
  ...
  private async setupEnvironmentsPersistence() {
    // Check if WorkspaceEnvironmentsService is handling persistence
    let workspaceEnvService: WorkspaceEnvironmentsService | null = null
    try {
      workspaceEnvService = this.bind(WorkspaceEnvironmentsService)
    } catch {
      // Service not registered, continue with Tauri Store persistence
    }
    ...
    try {
      if (E.isRight(loadResult)) {
        ...
        if (result.success) {
          ...
          // Only load from Tauri Store if WorkspaceEnvironmentsService is not active
          // WorkspaceEnvironmentsService will load from workspace files when ready
          if (!workspaceEnvService?.hasWorkspace) {
    ...
    environments$.subscribe(async (envs) => {
      // Skip Tauri Store persistence when WorkspaceEnvironmentsService is handling it
      // 检查 hasWorkspace 或 isLoading，确保在 workspace 模式下不会写入 Tauri Store
      if (workspaceEnvService?.hasWorkspace || workspaceEnvService?.isLoading) {
        return
      }
  ...
  private async setupSelectedEnvPersistence() {
    ...
    try {
      if (E.isRight(loadResult)) {
        ...
        if (result.success) {
          if (result.data !== null) {
            setSelectedEnvironmentIndex(result.data as SelectedEnvironmentIndex)
          } else {
      ...
      if (E.isRight(loadResult) && loadResult.right) {
        ...
        if (result.success) {
          ...
          this.restTabService.loadTabsFromPersistedState(
            result.data as PersistableTabState<AukTabDocument>
          )
        } else {
      ...
      if (E.isRight(loadResult) && loadResult.right) {
        ...
        if (result.success) {
          ...
          this.gqlTabService.loadTabsFromPersistedState(
            result.data as PersistableTabState<AukGQLDocument>
          )
        } else {
  ...
  public async setupFirst() {
    ...
    // Load local state early so FirstRunWizard can check FIRST_RUN_COMPLETED before app mount
