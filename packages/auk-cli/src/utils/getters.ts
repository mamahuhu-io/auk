...
export const getEffectiveFinalMetaData = (
  metaData: AukRESTHeader[] | AukRESTParam[],
  ...
  pipe(
    ...
    E.fromPredicate(
      /**
       * Check if every key-value is right either. Else return AukCLIError with
       * appropriate reason.
       */
...
export const getResourceContents = async (
  ...
  return readJsonFile(pathOrId, fileExistsInPath);
};
 ...
 *
 * @param {AukRESTRequestVariables} requestVariables - Incoming request variables.
 * @param {EnvironmentVariable[]} environmentVariables - Incoming environment variables.
 * @param {AukCollectionVariable[]} collectionVariables - Optional collection variables to be included.
