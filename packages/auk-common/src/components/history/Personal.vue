      ...
      <div class="flex">
        ...
        <div class="flex">
          <AukButtonSecondary
            ...
            to="https://auk.mamahuhu.dev/documentation/features/history"
            blank
            :title="t('app.wiki')"
            :icon="IconHelpCircle"
          />
...
import { AukGQLRequest, AukRESTRequest } from "@auk/data"
...
const getAppropriateURL = (entry: HistoryEntry) => {
  if (props.page === "rest") {
    return (entry.request as AukRESTRequest).endpoint
  } else if (props.page === "graphql") {
    return (entry.request as AukGQLRequest).url
  }
}
...
const addToCollection = (entry: HistoryEntry) => {
  if (props.page === "rest") {
    invokeAction("request.save-as", {
      requestType: "rest",
      request: entry.request as AukRESTRequest,
    })
  }
}
