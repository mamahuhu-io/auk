...
import { parseTemplateStringE, AukRESTRequestVariables } from "@auk/data"
...
import {
  ENV_VAR_NAME_REGEX,
  AUK_ENVIRONMENT_REGEX,
} from "~/helpers/environment-regex"

const AUK_ENV_HIGHLIGHT =
  "cursor-help transition rounded px-1 focus:outline-none mx-0.5 env-highlight"
const AUK_REQUEST_VARIABLE_HIGHLIGHT = "request-variable-highlight"
const AUK_COLLECTION_ENVIRONMENT_HIGHLIGHT = "collection-variable-highlight"
const AUK_ENVIRONMENT_HIGHLIGHT = "environment-variable-highlight"
const AUK_GLOBAL_ENVIRONMENT_HIGHLIGHT = "global-variable-highlight"
const AUK_ENV_HIGHLIGHT_NOT_FOUND = "environment-not-found-highlight"
...
const cursorTooltipField = (aggregateEnvs: AggregateEnvironment[]) =>
  hoverTooltip(
    (view, pos, side) => {
      ...
      //   wordSelection.to + 2
      // )
      // if (!AUK_ENVIRONMENT_REGEX.test(word)) return null
...
function checkEnv(env: string, aggregateEnvs: AggregateEnvironment[]) {
  let className = AUK_ENV_HIGHLIGHT_NOT_FOUND
  const envSource = aggregateEnvs.find(
    (k: { key: string }) => k.key === env.slice(2, -2)
  )?.sourceEnv

  if (envSource === "RequestVariable")
    className = AUK_REQUEST_VARIABLE_HIGHLIGHT
  else if (envSource === "CollectionVariable")
    className = AUK_COLLECTION_ENVIRONMENT_HIGHLIGHT
  else if (envSource === "Global") className = AUK_GLOBAL_ENVIRONMENT_HIGHLIGHT
  else if (envSource !== undefined) className = AUK_ENVIRONMENT_HIGHLIGHT

  return Decoration.mark({ class: `${AUK_ENV_HIGHLIGHT} ${className}` })
}

const getMatchDecorator = (aggregateEnvs: AggregateEnvironment[]) =>
  new MatchDecorator({
    regexp: AUK_ENVIRONMENT_REGEX,
...
const getRequestAndCollectionVariables = (
  ...
  collectionVariables: AukInheritedProperty["variables"]
) => {
...
export class AukEnvironmentPlugin {
...
export class AukReactiveEnvPlugin {
  private compartment = new Compartment()
  private envs: AggregateEnvironment[] = []
