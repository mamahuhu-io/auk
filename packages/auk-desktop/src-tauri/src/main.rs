...
fn main() {
    ...
    println!("Starting Auk Desktop in PORTABLE mode");

    #[cfg(not(feature = "portable"))]
    println!("Starting Auk Desktop in STANDARD mode");
    ...
    let log_dir = match path::logs_dir() {
        ...
        Err(e) => {
            ...
            println!("Starting Auk Desktop without logging...");
            return auk_desktop_lib::run();
        }
    };
    ...
    {
        tracing::info!(
            "Auk Desktop v{} starting in PORTABLE mode",
    ...
    {
        tracing::info!(
            "Auk Desktop v{} starting in STANDARD mode",
            env!("CARGO_PKG_VERSION")
        );
        if let Ok(config_dir) = path::config_dir() {
