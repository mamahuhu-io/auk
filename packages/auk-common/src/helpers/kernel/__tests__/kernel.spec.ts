...
import { MediaType } from "@auk/kernel"
import { AukGQLRequest } from "@auk/data"
import { EffectiveAukRESTRequest } from "~/helpers/utils/EffectiveURL"
import { RelayResponse } from "@auk/kernel"
...
describe("GraphQL Request Transformation", () => {
  const baseRequest: AukGQLRequest = {
    v: 9,
    name: "Test Query",
    url: "https://api.example.com/graphql",
    headers: [],
...
describe("REST Response Transformation", () => {
  it("transforms a successful REST response", async () => {
    ...
    const originalRequest: AukRESTRequest = {
      v: "15",
...
describe("REST Request Transformation", () => {
  const baseEffectiveRequest: EffectiveAukRESTRequest = {
    v: "15",
    name: "Test Request",
    method: "GET",
    endpoint: "https://api.example.com",
  ...
  it("includes auth parameters when basic auth is active", async () => {
    const request: EffectiveAukRESTRequest = {
