 ...
 * across pm.environment and auk.env namespaces.
 */
...
// Testing: pm.environment (active scope) and auk.env
...
const NAMESPACES = [
  ...
  { name: "auk.env", get: "auk.env.get", set: "auk.env.set" },
] as const
...
describe("Environment Variable Fallback Behavior - All Namespaces", () => {
  ...
  describe.each(NAMESPACES)(
    ...
    ({ name, get }) => {
      test("should fallback to initialValue in global scope", async () => {
        ...
        // Use auk.env.global.get for auk, pm.globals.get for pm
        const globalGet =
          name === "pm.environment" ? "pm.globals.get" : "auk.env.global.get"
  ...
  // Auk namespace returns null for non-existent keys
  describe("auk.env - non-existent variable behavior", () => {
    test("should return null for non-existent variable (not fallback)", async () => {
      ...
      const result = await runTest(
        `
        const value = auk.env.get("nonExistent")
