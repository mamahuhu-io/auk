...
import { AukGQLAuth } from "../../graphql/v/5"
import { AukRESTRequest } from "../../rest"
import { AukRESTHeaders } from "../../rest/v/1"
import { AukRESTAuth } from "../../rest/v/5"
...
export const v2_baseCollectionSchema = z.object({
  ...
  headers: z.union([AukRESTHeaders, z.array(GQLHeader)]),
})
...
export const V2_SCHEMA = v2_baseCollectionSchema.extend({
  folders: z.lazy(() => z.array(entityRefUptoVersion(AukCollection, 2))),
  ...
  up(old: z.infer<typeof V1_SCHEMA>) {
    const result: z.infer<typeof V2_SCHEMA> = {
      ...
      folders: old.folders.map((folder) => {
        const result = AukCollection.safeParseUpToVersion(folder, 2)
