...
import { addGraphqlHistoryEntry, makeGQLHistoryEntry } from "~/store/history"

import { KernelInterceptorService } from "~/services/kernel-interceptor.service"
import { GQLTabService } from "~/services/tab/graphql"

import { MediaType, content, Method, RelayRequest } from "@auk/kernel"
...
type ConnectionRequestOptions = {
  ...
  inheritedHeaders: AukGQLRequest["headers"]
  inheritedAuth?: AukGQLAuth
}
...
const getSchema = async (options: ConnectionRequestOptions) => {
  try {
    ...
    let runHeaders: AukGQLRequest["headers"] = []
...
export const runGQLOperation = async (options: RunQueryOptions) => {
  ...
  const finalAukHeaders: AukRESTHeaders = Object.entries(finalHeaders).map(
    ([key, value]) => ({
      active: true,
      key,
      value,
  ...
  const gqlRequest: AukGQLRequest = {
    v: 9,
    name: options.name || "Untitled Request",
    url: finalUrl,
    headers: finalAukHeaders,
    query,
    variables,
  ...
  addGraphqlHistoryEntry(
    makeGQLHistoryEntry({
      request: makeGQLRequest({
        ...
        variables,
        auth: request.auth as AukGQLAuth,
      }),
      response,
      star: false,
    })
