...
pub const APP_ID: &str = "io.mamahuhu.auk";
...
pub fn bundle_path() -> PathBuf {
    if cfg!(feature = "portable") {
        ...
        std::env::temp_dir().join("auk_bundle.zip")
    }
}
...
pub fn manifest_path() -> PathBuf {
    if cfg!(feature = "portable") {
        std::env::current_dir()
            .unwrap_or_else(|_| std::env::temp_dir())
            .join("auk_manifest.json")
    } else {
        std::env::temp_dir().join("auk_manifest.json")
    }
}
...
mod tests {
    ...
    fn test_app_id_constant() {
        assert_eq!(APP_ID, "io.mamahuhu.auk");
    }

    #[test]
    fn test_log_file_path() {
        let path = log_file_path();
        assert!(path.to_string_lossy().contains("io.mamahuhu.auk.log"));
    }

    #[test]
    fn test_manifest_path_portable() {
        let path = manifest_path();
        assert!(path.to_string_lossy().ends_with("auk_manifest.json"));
    }
...
mod portable_tests {
    ...
    fn test_portable_bundle_path() {
        ...
        assert!(path.file_name().unwrap() == "auk_bundle.zip");
    }
    ...
    fn test_portable_manifest_path() {
        let path = manifest_path();
        assert!(path.file_name().unwrap() == "auk_manifest.json");
    }
