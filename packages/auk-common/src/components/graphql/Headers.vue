  ...
  >
    ...
    <div class="flex">
      <AukButtonSecondary
        ...
        to="https://auk.mamahuhu.dev/documentation/features/graphql-api-testing"
        blank
        :title="t('app.wiki')"
        :icon="IconHelpCircle"
      />
...
const props = defineProps<{
  modelValue: AukGQLRequest
  isCollectionProperty?: boolean
  inheritedProperties?: AukInheritedProperty
}>()

const emit = defineEmits<{
  (e: "update:modelValue", value: AukGQLRequest): void
...
const getComputedAuthHeaders = async (
  ...
  if (request.auth.authType === "basic") {
    ...
    if (addTo === "HEADERS") {
      ...
      const { url } = req as AukGQLRequest
...
const getComputedHeaders = async (req: AukGQLRequest) => {
  return [
  ...
  async () => {
    ...
    if (
      ...
      const [computedAuthHeader] = await getComputedAuthHeaders(
        request.value,
        props.inheritedProperties.auth.inheritedAuth as AukGQLAuth
      )
