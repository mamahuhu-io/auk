...
export const AukCollection = createVersionedEntity({
  latestVersion: 11,
  versionMap: {
    1: V1_VERSION,
    2: V2_VERSION,
...
export type AukCollection = InferredEntity<typeof AukCollection>

export type AukCollectionVariable = InferredEntity<
  typeof AukCollection
>["variables"][number]
 ...
 */
export function makeCollection(x: Omit<AukCollection, "v">): AukCollection {
  return {
 ...
 */
export function translateToNewRESTCollection(x: any): AukCollection {
  // Legacy
 ...
 * @returns The proper new collection format
 */
export function translateToNewGQLCollection(x: any): AukCollection {
  // Legacy
  const name = x.name ?? "Untitled"
