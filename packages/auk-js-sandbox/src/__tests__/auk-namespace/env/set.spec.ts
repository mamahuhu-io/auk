...
describe("auk.env.set", () => {
  ...
  test("creates non-existent key in selected environment", () => {
    return expect(
      execEnv(`auk.env.set("a", "created")`, { global: [], selected: [] })()
    ).resolves.toEqualRight(
      expect.objectContaining({
        selected: [
          {
  ...
  test("does not affect secret status for existing variable", () => {
    return expect(
      execEnv(`auk.env.set("mysecret", "not-secret-anymore")`, {
        selected: [
          {
            key: "mysecret",
  ...
  test("rejects non-string key", () => {
    return expect(
      execEnv(`auk.env.set(7, "foo")`, { selected: [], global: [] })()
    ).resolves.toBeLeft()
  })

  test("rejects non-string value", () => {
    return expect(
      execEnv(`auk.env.set("key", 123)`, { selected: [], global: [] })()
    ).resolves.toBeLeft()
  })

  test("both key and value must be strings", () => {
    return expect(
      execEnv(`auk.env.set(5, 6)`, { selected: [], global: [] })()
    ).resolves.toBeLeft()
  })
...
describe("auk.env.active.set", () => {
  test("sets in selected even if key exists in global", () => {
    return expect(
      execEnv(`auk.env.active.set("b", "in-selected")`, {
        selected: [],
        global: [
          {
            key: "b",
  ...
  test("updates existing selected variable", () => {
    return expect(
      execEnv(`auk.env.active.set("foo", "bar")`, {
        selected: [
          {
            key: "foo",
            currentValue: "baz",
  ...
  test("creates a new key in selected", () => {
    return expect(
      execEnv(`auk.env.active.set("make", "new")`, {
        selected: [],
        global: [],
      })()
    ).resolves.toEqualRight(
  ...
  test("rejects non-string key", () => {
    return expect(
      execEnv(`auk.env.active.set(null, "value")`, {
        selected: [],
        global: [],
      })()
    ).resolves.toBeLeft()
  ...
  test("rejects non-string value", () => {
    return expect(
      execEnv(`auk.env.active.set("key", {})`, { selected: [], global: [] })()
    ).resolves.toBeLeft()
  })
})

describe("auk.env.global.set", () => {
  test("creates in global, does not affect selected", () => {
    return expect(
      execEnv(`auk.env.global.set("c", "in-global")`, {
        selected: [
          {
            key: "c",
  ...
  test("updates existing global variable", () => {
    return expect(
      execEnv(`auk.env.global.set("d", "updated")`, {
        selected: [],
        global: [
  ...
  test("creates new variable in global", () => {
    return expect(
      execEnv(`auk.env.global.set("e", "new-value")`, {
        selected: [],
        global: [],
      })()
    ).resolves.toEqualRight(
  ...
  test("rejects non-string key", () => {
    return expect(
      execEnv(`auk.env.global.set([], "foo")`, { selected: [], global: [] })()
    ).resolves.toBeLeft()
  })

  test("rejects non-string value", () => {
    return expect(
      execEnv(`auk.env.global.set("key", true)`, {
        selected: [],
        global: [],
      })()
    ).resolves.toBeLeft()
