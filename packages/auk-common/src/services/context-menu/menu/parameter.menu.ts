...
export class ParameterMenuService extends Service implements ContextMenu {
  ...
  private readonly tabService = this.bind(RESTTabService)
  ...
  private addParameter(text: string) {
    ...
    if (this.tabService.currentActiveTab.value.document.type === "test-runner")
      return

    const currentActiveRequest =
      this.tabService.currentActiveTab.value.document.type === "request"
        ? this.tabService.currentActiveTab.value.document.request
        : this.tabService.currentActiveTab.value.document.response
            .originalRequest
