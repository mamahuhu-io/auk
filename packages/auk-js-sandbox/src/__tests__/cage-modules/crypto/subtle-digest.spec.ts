...
describe("crypto.subtle.digest()", () => {
  test("should compute SHA-256 hash of string data", () => {
    const script = `
      ...
      auk.test("SHA-256 digest is correct", () => {
        auk.expect(hashHex).toBe(expectedHash)
        auk.expect(hashBuffer.byteLength).toBe(32) // SHA-256 produces 32 bytes
      })
    `
  ...
  test("should compute SHA-1 hash", () => {
    const script = `
      ...
      auk.test("SHA-1 digest is correct", () => {
        auk.expect(hashHex).toBe(expectedHash)
        auk.expect(hashBuffer.byteLength).toBe(20) // SHA-1 produces 20 bytes
      })
    `
  ...
  test("should compute SHA-384 hash", () => {
    const script = `
      ...
      auk.test("SHA-384 produces correct byte length", () => {
        auk.expect(hashBuffer.byteLength).toBe(48) // SHA-384 produces 48 bytes
      })
    `
  ...
  test("should compute SHA-512 hash", () => {
    const script = `
      ...
      auk.test("SHA-512 produces correct byte length", () => {
        auk.expect(hashBuffer.byteLength).toBe(64) // SHA-512 produces 64 bytes
      })
    `
  ...
  test("should handle empty string", () => {
    const script = `
      ...
      auk.test("Empty string digest is correct", () => {
        auk.expect(hashHex).toBe(expectedHash)
      })
    `
  ...
  test("should produce consistent hashes for same input", () => {
    const script = `
      ...
      auk.test("Same input produces same hash", () => {
        auk.expect(hash1Hex).toBe(hash2Hex)
      })
    `
  ...
  test("should produce different hashes for different inputs", () => {
    const script = `
      ...
      auk.test("Different inputs produce different hashes", () => {
        auk.expect(hash1Hex).not.toBe(hash2Hex)
      })
    `
