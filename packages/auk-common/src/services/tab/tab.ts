...
{
  protected tabMap = reactive(new Map<string, AukTab<Doc>>()) as Map<
    string,
    AukTab<Doc>
  ...
  protected recentlyClosedTabs: Array<{ tab: AukTab<Doc>; index: number }> = []
  ...
  public createNewTab(document: Doc, switchToIt = true): AukTab<Doc> {
    const id = this.generateNewTabID()

    const tab: AukTab<Doc> = { id, document }
  ...
  public getTabs(): AukTab<Doc>[] {
    return Array.from(this.tabMap.values())
  }

  public getActiveTab(): AukTab<Doc> | null {
  ...
  public getActiveTabs(): Readonly<ComputedRef<AukTab<Doc>[]>> {
  ...
  public updateTab(tabUpdate: AukTab<Doc>) {
  ...
  private addToRecentlyClosedTabs(tab: AukTab<Doc>, index: number): void {
  ...
  public getTabsRefTo(func: (tab: AukTab<Doc>) => boolean) {
    return Array.from(this.tabMap.values())
      .filter(func)
