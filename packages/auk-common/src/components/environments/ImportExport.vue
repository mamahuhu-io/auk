...
import { aukEnvImporter } from "~/helpers/import-export/import/aukEnv"
...
const AukEnvironmentsImport: ImporterOrExporter = {
  metadata: {
    id: "import.from_json",
  ...
  component: FileSource({
    ...
    caption: "import.auk_environment_description",
    onImportFromFile: async (environments) => {
      isRESTImporterInProgress.value = true

      const res = await aukEnvImporter(environments)()
...
const EnvironmentsImportFromGIST: ImporterOrExporter = {
  ...
  component: GistSource({
    ...
    onImportFromGist: async (environments) => {
      ...
      const res = await aukEnvImporter(environments.right)()
...
const AukEnvironmentsExport: ImporterOrExporter = {
  metadata: {
    id: "export.as_json",
    name: "export.as_json",
  ...
  action: async () => {
    ...
    platform.analytics?.logEvent({
      type: "AUK_EXPORT_ENVIRONMENT",
      platform: "rest",
    })
  },
}

const AukEnvironmentsGistExporter: ImporterOrExporter = {
  metadata: {
    id: "export.as_gist",
...
const exporterModules = computed(() => {
  const enabledExporters = [AukEnvironmentsExport]

  if (platform.platformFeatureFlags.exportAsGIST) {
    enabledExporters.push(AukEnvironmentsGistExporter)
  }
