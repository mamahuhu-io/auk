...
// Removes the OpenAPI Path Templating to the Auk Templating (<< ? >>)
const replaceOpenApiPathTemplating = flow(
  S.replace(/{/g, "<<"),
  S.replace(/}/g, ">>")
)

const parseOpenAPIParams = (params: OpenAPIParamsType[]): AukRESTParam[] =>
  pipe(
    params,
...
const parseOpenAPIV3Responses = (
  ...
  originalRequest: AukRESTResponseOriginalRequest
): AukRESTRequestResponses => {
  const responses = op.responses
  if (!responses) return {}

  const res: AukRESTRequestResponses = {}
...
const parseOpenAPIHeaders = (params: OpenAPIParamsType[]): AukRESTHeader[] =>
  pipe(
    params,
...
const parseOpenAPIV2Body = (op: OpenAPIV2.OperationObject): AukRESTReqBody => {
  const obj = (op.consumes ?? [])[0] as string | undefined

  // Not a content-type Auk supports
...
const resolveOpenAPIV3Security = (
  ...
  // NOTE: Auk only considers the first security requirement
...
const convertOpenApiDocsToAuk = (
  docs: OpenAPI.Document[]
): TE.TaskEither<string, AukCollection[]> => {
  ...
  const collections = docs.map((doc) => {
    ...
    const paths = Object.entries(doc.paths ?? {})
      .map(([pathName, pathObj]) =>
        convertPathToAukReqs(doc, pathName, pathObj)
      )
      .flat()

    const requestsByTags: Record<string, Array<AukRESTRequest>> = {}
    const requestsWithoutTags: Array<AukRESTRequest> = []
...
export const aukOpenAPIImporter = (fileContents: string[]) =>
  pipe(
    ...
    TE.chainW(convertOpenApiDocsToAuk)
  )
