...
export const aukRESTImporter = (content: string[]) =>
  pipe(
    content,
...
/**
 * checks if a collection is a valid Auk collection.
 * else translate it into one.
 */
const validateCollection = (collection: unknown) => {
  const collectionSchemaParsedResult = AukCollection.safeParse(collection)

  if (collectionSchemaParsedResult.type === "ok") {
    const requests = collectionSchemaParsedResult.value.requests.map(
      (request) => {
        const requestSchemaParsedResult = AukRESTRequest.safeParse(request)
...
export const aukGQLImporter = (content: string) =>
  pipe(
...
export const validateGQLCollection = (collection: unknown) => {
  ...
  if (collectionSchemaParsedResult.type === "ok") {
    const requests = collectionSchemaParsedResult.value.requests.map(
      (request) => {
        const requestSchemaParsedResult = AukGQLRequest.safeParse(request)
