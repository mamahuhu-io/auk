...
impl<'a> ContentHandler<'a> {
    pub(crate) fn new(handle: &'a mut Easy, headers: &'a HashMap<String, String>) -> Self {
        tracing::debug!("Creating new ContentHandler with headers: {:?}", headers);
        Self { handle, headers }
    }

    /// Check if Content-Type header is already set by the user
    fn has_content_type(&self) -> bool {
        self.headers.iter().any(|(k, _)| k.to_lowercase() == "content-type")
    }

    /// Set Content-Type header if not already set by user
    fn set_content_type_if_needed(&mut self, media_type: &MediaType) -> Result<()> {
        if !self.has_content_type() {
            tracing::debug!(media_type = %media_type, "Setting Content-Type header (not provided by user)");
            ...
            self.handle.http_headers(list).map_err(|e| {
                tracing::error!(error = %e, "Failed to apply Content-Type header");
                RelayError::Network {
                    message: "Failed to apply Content-Type header".into(),
                    cause: Some(e.to_string()),
                }
            })?;
        } else {
            tracing::debug!("Content-Type already set by user, skipping default");
        }
        Ok(())
    }
    ...
    fn set_text_content(&mut self, content: &str, media_type: &MediaType) -> Result<()> {
        ...
        self.set_content_type_if_needed(media_type)?;
    ...
    fn set_binary_content(
        ...
        if let Some(name) = filename {
            ...
            let mut list = curl::easy::List::new();
            list.append(&format!("Content-Disposition: attachment; filename=\"{}\"", safe_name))
                .map_err(|e| {
                    tracing::error!(error = %e, "Failed to set Content-Disposition header");
                    RelayError::Network {
                        message: "Failed to set Content-Disposition header".into(),
                        cause: Some(e.to_string()),
                    }
                })?;

            self.handle.http_headers(list).map_err(|e| {
                tracing::error!(error = %e, "Failed to apply Content-Disposition header");
                RelayError::Network {
                    message: "Failed to apply Content-Disposition header".into(),
                    cause: Some(e.to_string()),
                }
            })?;
        }
    ...
    fn set_form_content(
        ...
        // Note: Form data sets its own Content-Type (multipart/form-data with boundary)
        // via curl's httppost API, so we don't manually set it here
