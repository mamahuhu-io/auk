...
export const getRequestSetterMethods = (request: AukRESTRequest) => {
  // Clone to allow safe mutations internally
  const updatedRequest = <AukRESTRequest>cloneDeep(request)
  ...
  const setHeaders = (headers: AukRESTHeaders) => {
    const parseResult = AukRESTHeaders.safeParse(headers)
  ...
  const setParams = (params: AukRESTParams) => {
    const parseResult = AukRESTParams.safeParse(params)
  ...
  const setBody = (newBody: Partial<AukRESTReqBody>) => {
    ...
    const parseResult = AukRESTReqBody.safeParse(mergedBody)
  ...
  const setAuth = (newAuth: AukRESTAuth) => {
    // Runtime validations given the input is user controlled
    if (
    ...
    const parseResult = AukRESTAuth.safeParse(mergedAuth)
