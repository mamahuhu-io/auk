...
import { AukActionWithOptionalArgs, invokeAction } from "./actions"
...
import { isDesktopPlatform, listenDesktopEvent } from "~/platform/capabilities"
...
const baseBindings: {
  [_ in ShortcutKey]?: AukActionWithOptionalArgs
} = {
...
export function hookKeybindingsListener() {
  onMounted(async () => {
    ...
    if (isDesktopPlatform()) {
      try {
        unlistenTauriEvent = await listenDesktopEvent<string>(
          "auk_desktop_shortcut",
          (payload) => {
            console.info("Tauri shortcut payload", payload)
            handleTauriShortcut(payload)
          }
        )
      } catch (error) {
