...
fn setup_deep_link_handler(app: &tauri::App) -> Result<(), AukError> {
...
fn setup_server(app: &tauri::App) -> Result<u16, AukError> {
    let server_port: u16 = PortPicker::new()
        .protocol(Protocol::Tcp)
        .port_range(15000..=25000)
        .pick()
        .map_err(|_| AukError::ServerPortInitialization)?;
    ...
    let port = *SERVER_PORT
        .get()
        .ok_or(AukError::ServerPortInitialization)?;
...
async fn setup_version_backup(app: &tauri::App) -> Result<(), AukError> {
...
/// Gracefully quit the Auk Desktop
///
...
pub fn run() {
    ...
    tracing::info!(mode = mode, "Auk Desktop running in {} mode", mode);
    ...
    let app = tauri::Builder::default()
        .setup(|app| {
            tauri::async_runtime::block_on(async {
                ...
                tracing::info!("Starting Auk Desktop v{}", env!("CARGO_PKG_VERSION"));
                Ok(())
            })
        })
        .plugin(
    ...
    if let Err(e) = app {
        tracing::error!(error = %e, "Error while running Auk Desktop");
    }
}
