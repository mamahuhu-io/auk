...
export type AuthEvent =
  | { event: "probable_login"; user: AukUser } // We have previous login state, but the app is waiting for authentication
  | { event: "login"; user: AukUser } // We are authenticated
  | { event: "logout" } // No authentication and we have no previous state

export type GithubSignInResult =
  | { type: "success"; user: AukUser } // The authentication was a success
...
export type AuthPlatformDef = {
   ...
   *
   * @returns An observable which returns a `AukUser` or null if not logged in (or login not completed)
   */
  getCurrentUserStream: () => Observable<AukUser | null>
   ...
   *
   * @returns An obsverable which returns a `AukUser` for the probable user (or confirmed user if authed) or null if we don't know about a probable user
   */
  getProbableUserStream: () => Observable<AukUser | null>
   ...
   */
  setDisplayName: (name: string) => Promise<E.Either<string, undefined>>
