...
function populateValues(
  variables: AukCollectionVariable[],
  parentID: string,
  showSecret: boolean
) {
...
export function cascadeParentCollectionForProperties(
  ...
  let auth: AukInheritedProperty["auth"] = {
    parentID: folderPath ?? "",
    parentName: "",
    inheritedAuth: {
      authType: "none",
  ...
  const headers: AukInheritedProperty["headers"] = []

  const variables: AukInheritedProperty["variables"] = []
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    const parentFolderAuth = parentFolder.auth as AukRESTAuth | AukGQLAuth
    const parentFolderHeaders = parentFolder.headers as
      | AukRESTHeaders
      | GQLHeader[]
    const parentFolderVariables =
      parentFolder.variables as AukCollectionVariable[]
...
const restCollectionDispatchers = defineDispatchers({
  ...
  addCollection(
    { state }: RESTCollectionStoreType,
    { collection }: { collection: AukCollection }
  ) {
    return {
  ...
  editCollection(
    { state }: RESTCollectionStoreType,
    {
      ...
      partialCollection: Partial<AukCollection>
    }
  ) {
    return {
  ...
  addFolder(
    ...
    const newFolder: AukCollection = makeCollection({
      name,
      folders: [],
      requests: [],
      auth: {
  ...
  moveFolder(
    ...
    if (destinationPath === null) {
      ...
      newState.push(theFolder[0] as AukCollection)
...
const gqlCollectionDispatchers = defineDispatchers({
  ...
  editCollection(
    { state }: GraphqlCollectionStoreType,
    {
      collectionIndex,
      collection,
    }: { collectionIndex: number; collection: Partial<AukCollection> }
  ) {
    return {
      state: state.map((col, index) =>
  ...
  editFolder(
    { state }: GraphqlCollectionStoreType,
    { path, folder }: { path: string; folder: Partial<AukCollection> }
  ) {
    const newState = state
...
export function setRESTCollections(entries: AukCollection[]) {
...
export function appendRESTCollections(entries: AukCollection[]) {
  restCollectionStore.dispatch({
...
export function addRESTCollection(collection: AukCollection) {
  restCollectionStore.dispatch({
...
function computeCollectionInheritedProps(
  ...
  parentAuth: AukRESTAuth | null = null,
  parentHeaders: AukRESTHeaders | null = null,
  parentVariables: AukCollectionVariable[] | null = null
): {
  auth: AukRESTAuth
  ...
  const inheritedHeaders: AukRESTHeaders = [
    ...(parentHeaders ?? []),
    ...collection.headers,
  ]
...
export function getRESTCollectionInheritedProps(
  collectionID: string,
  collections: AukCollection[] = restCollectionStore.value.state,
  type: "my-collections" = "my-collections"
): {
  auth: AukRESTAuth
  headers: AukRESTHeaders
...
export function editRESTFolder(path: string, folder: Partial<AukCollection>) {
  restCollectionStore.dispatch({
...
export function saveRESTRequestAs(path: string, request: AukRESTRequest) {
...
export function setGraphqlCollections(entries: AukCollection[]) {
...
export function appendGraphqlCollections(entries: AukCollection[]) {
  graphqlCollectionStore.dispatch({
    dispatcher: "appendCollections",
    payload: {
      entries,
...
export function addGraphqlCollection(collection: AukCollection) {
  graphqlCollectionStore.dispatch({
    dispatcher: "addCollection",
    payload: {
      collection,
...
export function editGraphqlCollection(
  collectionIndex: number,
  collection: Partial<AukCollection>
) {
  graphqlCollectionStore.dispatch({
    dispatcher: "editCollection",
    payload: {
...
export function saveGraphqlRequestAs(path: string, request: AukGQLRequest) {
  // For calculating the insertion request index
  const targetLocation = navigateToFolderWithIndexPath(
