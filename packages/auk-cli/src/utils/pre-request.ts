...
import { runPreRequestScript } from "@auk/js-sandbox/node";
import { createAukFetchHook } from "./auk-fetch";
 ...
 * @param request AukRESTRequest to be converted to EffectiveAukRESTRequest.
 * @param envs Environment variables related to request.
 * @returns EffectiveAukRESTRequest that includes parsed ENV variables with in
 * request OR AukCLIError with error code and related information.
 */
export const preRequestScriptRunner = (
  ...
  AukCLIError,
  { effectiveRequest: EffectiveAukRESTRequest } & { updatedEnvs: AukEnvs }
> => {
  const experimentalScriptingSandbox = !legacySandbox;
  const aukFetchHook = createAukFetchHook();
...
> {
  ...
  if (request.auth.authActive) {
    ...
    if (request.auth.authType === "basic") {
      ...
      const { method, endpoint } = request as AukRESTRequest;
 ...
 * Or, AukCLIError in case of error while parsing.
 */
function getFinalBodyFromRequest(
  request: AukRESTRequest,
  resolvedVariables: EnvironmentVariable[]
): E.Either<AukCLIError, string | null | FormData | File> {
 ...
 * PRE_REQUEST_ERROR code in given auk-error list.
