...
import { AUK_SUPPORTED_PREDEFINED_VARIABLES } from "@auk/data"

import IconSquareAsterisk from "~icons/lucide/square-asterisk?raw"
import { isComment } from "./helpers"

const AUK_PREDEFINED_VARIABLES_REGEX = /(<<\$[a-zA-Z0-9-_]+>>)/g

const AUK_PREDEFINED_VARIABLE_HIGHLIGHT =
  "cursor-help transition rounded px-1 focus:outline-none mx-0.5 predefined-variable-highlight"
const AUK_PREDEFINED_VARIABLE_HIGHLIGHT_VALID = "predefined-variable-valid"
const AUK_PREDEFINED_VARIABLE_HIGHLIGHT_INVALID = "predefined-variable-invalid"

const getMatchDecorator = () => {
  return new MatchDecorator({
    regexp: AUK_PREDEFINED_VARIABLES_REGEX,
...
const cursorTooltipField = () =>
  hoverTooltip(
    (view, pos, side) => {
      ...
      //   wordSelection.to + 2
      // )
      // if (!AUK_PREDEFINED_VARIABLES_REGEX.test(word)) return null
      ...
      if (
        (start === pos && side < 0) ||
        (end === pos && side > 0) ||
        !AUK_PREDEFINED_VARIABLES_REGEX.test(
      ...
      const variable = AUK_SUPPORTED_PREDEFINED_VARIABLES.find(
...
const checkPredefinedVariable = (variable: string) => {
  ...
  const className = AUK_SUPPORTED_PREDEFINED_VARIABLES.find((v) => {
    return v.key === inputVariableKey
  })
    ? AUK_PREDEFINED_VARIABLE_HIGHLIGHT_VALID
    : AUK_PREDEFINED_VARIABLE_HIGHLIGHT_INVALID

  return Decoration.mark({
    class: `${AUK_PREDEFINED_VARIABLE_HIGHLIGHT} ${className}`,
  })
}
...
export class AukPredefinedVariablesPlugin {
