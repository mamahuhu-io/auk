import {
  ...
  AUK_SUPPORTED_PREDEFINED_VARIABLES,
} from "@auk/data"
...
import DispatchingStore, { defineDispatchers } from "~/store/DispatchingStore"
import { CurrentValueService } from "~/services/current-environment-value.service"
import { SecretEnvironmentService } from "~/services/secret-environment.service"

export type SelectedEnvironmentIndex =
  | { type: "NO_ENV_SELECTED" }
  | { type: "WORKSPACE_ENV"; index: number }
...
const dispatchers = defineDispatchers({
  setSelectedEnvironmentIndex(
    ...
    if (selectedEnvironmentIndex.type === "WORKSPACE_ENV") {
  ...
  environmentsStore.subject$.pipe(
    map(({ environments, selectedEnvironmentIndex }) => {
      if (selectedEnvironmentIndex.type === "NO_ENV_SELECTED") {
        ...
        return env
      } else if (selectedEnvironmentIndex.type === "WORKSPACE_ENV") {
...
export const aggregateEnvs$: Observable<AggregateEnvironment[]> = combineLatest(
  ...
  map(([selectedEnv, globalEnv]) => {
    ...
    AUK_SUPPORTED_PREDEFINED_VARIABLES.forEach(({ key, getValue }) => {
...
export function getAggregateEnvs() {
  ...
  return [
    ...AUK_SUPPORTED_PREDEFINED_VARIABLES.map(({ key, getValue }) => {
...
export function getCurrentEnvironment(): Environment {
  if (
    ...
    environmentsStore.value.selectedEnvironmentIndex.type === "WORKSPACE_ENV"
  ) {
