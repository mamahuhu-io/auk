import { AukModule, AUK_MODULES } from "."
import {
  createRouter,
  createWebHistory,
...
export default <AukModule>{
  onVueAppInit(app) {
    ...
    AUK_MODULES.forEach((mod) => mod.onRouterInit?.(app, router))
    platform.addedAukModules?.forEach((mod) => mod.onRouterInit?.(app, router))
  },
}
