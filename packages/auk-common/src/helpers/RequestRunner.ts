...
import { createAukFetchHook } from "~/helpers/auk-fetch"
...
import { AukTestData, AukTestResult } from "./types/AukTestResult"
...
export const getTestableBody = (
  res: AukRESTResponse & { type: "success" | "fail" }
) => {
  const contentTypeHeader = res.headers.find(
    (h: AukRESTHeader) => h.key.toLowerCase() === "content-type"
  )
...
export const executedResponses$ = new Subject<
  AukRESTResponse & { type: "success" | "fail " }
>()
...
const delegatePreRequestScriptRunner = (
  ...
  const aukFetchHook = createAukFetchHook(kernelInterceptorService)
...
export function runRESTRequest$(
  tab: Ref<AukTab<AukRequestDocument>>
): [
  () => void,
  Promise<
    | E.Left<"script_fail" | "cancellation">
    | E.Right<Observable<AukRESTResponse>>
  >,
] {
  ...
  const requestHeaders: AukRESTHeaders = [
    ...(inheritedHeaders ?? []),
    ...request.headers,
  ]
...
function updateEnvsAfterTestScript(
  ...
  if (initialEnvironmentIndex.type === "WORKSPACE_ENV") {
  ...
  // Team environment updates not supported in local-only mode
}
...
export async function runTestRunnerRequest(
  ...
  | E.Right<{
      response: AukRESTResponse
      testResult: AukTestResult
      updatedRequest: AukRESTRequest
    }>
  | undefined
> {
  const cookieJarEntries = getCookieJarEntries()
...
function translateToSandboxTestResults(
  ...
  const translateChildTests = (child: TestDescriptor): AukTestData => {
    return {
