...
import { AukCollection, AukGQLRequest, AukRESTRequest } from "@auk/data"
    ...
    if (type === "rest") {
      ...
      if (possibleTab) {
        ...
        const req = this.getRESTFolderFromFolderPath(folderPath.join("/"))
          ?.requests[reqIndex] as AukRESTRequest
      ...
      const req = this.getGQLFolderFromFolderPath(folderPath.join("/"))
        ?.requests[reqIndex] as AukGQLRequest
