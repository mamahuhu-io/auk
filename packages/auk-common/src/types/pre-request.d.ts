...
interface AukRESTReqBody {
  contentType: AukRESTContentType
...
interface AukRESTAuthNone {
  authType: "none"
  authActive: boolean
}

interface AukRESTAuthInherit {
  authType: "inherit"
  authActive: boolean
}

interface AukRESTAuthBasic {
  authType: "basic"
  authActive: boolean
  username: string
  password: string
...
interface AukRESTAuthBearer {
  authType: "bearer"
  authActive: boolean
  token: string
}

interface AukRESTAuthAPIKey {
  authType: "api-key"
  authActive: boolean
  key: string
  value: string
...
interface AukRESTAuthAWSSignature {
...
interface AukRESTAuthAkamaiEdgeGrid {
  authType: "akamai-eg"
  authActive: boolean
  accessToken: string
  clientToken: string
...
declare namespace auk {
  ...
  const request: Readonly<{
    ...
    readonly params: AukRESTParam[]
    readonly headers: AukRESTHeader[]
    readonly body: AukRESTReqBody
    readonly auth: AukRESTAuth
    setUrl(url: string): void
    setMethod(method: string): void
    setHeader(name: string, value: string): void
    setHeaders(headers: Array<Partial<AukRESTHeader>>): void
    removeHeader(key: string): void
    setParam(name: string, value: string): void
    setParams(params: Array<Partial<AukRESTParam>>): void
     ...
     *
     * @param body - Partial or complete AukRESTReqBody object
     *`
     * @example
     * // Partial update - just change content type
     * auk.request.setBody({ contentType: "application/xml" })
     *
     * // Partial update - just change body content
     * auk.request.setBody({ body: JSON.stringify({ updated: true }) })
     *
     ...
     * })
     */
    setBody(body: Partial<AukRESTReqBody>): void
    /**
     ...
     *
     * @param auth - Partial or complete AukRESTAuth object
     *
     * @example
     * // Partial update - just change the token (merges with existing)
     * auk.request.setAuth({ bearerToken: "new-token" })
     *
     ...
     * })
     */
    setAuth(auth: Partial<AukRESTAuth>): void
    variables: Readonly<{
      get(key: string): string | null
      set(key: string, value: string): void
    }>
...
/**
 * Global fetch function - alias to auk.fetch()
...
declare namespace pm {
  ...
  let request: {
     ...
     * Returns undefined in Auk as certificate configuration is handled at the application level
     ...
     * Returns undefined in Auk as proxy configuration is handled at the application level
    ...
    // Uses AukRESTReqBody for type safety with Postman's update() extension
    body: AukRESTReqBody & {
      update(
        body:
          | string
   ...
   * @throws Error - sendRequest is not supported in Auk
   */
   ...
   * Collection variables are not supported in Auk as they are a Postman Workspace feature
   */
   ...
   * Vault is not supported in Auk as it is a Postman-specific feature
   */
   ...
   * Iteration data is not supported in Auk as it requires Collection Runner
   */
   ...
   * This feature is not supported in Auk as it requires a browser-based visualization UI.
  ...
  const visualizer: Readonly<{
     ...
     * @throws Error - Visualizer is not supported in Auk
     */
    set(
      layout: string,
  ...
  const execution: Readonly<{
     ...
     * Always returns ["Auk"] with current = "Auk"
     */
   ...
   * @param packageName - Name of the package to import (e.g., '@org/package-name' or 'npm:package-name@version')
   * @returns The imported package module
   * @throws Error - Package imports are not supported in Auk
   */
