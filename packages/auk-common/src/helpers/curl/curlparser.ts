 ...
 */
import { FormDataKeyValue, AukRESTReqBody, makeRESTRequest } from "@auk/data"
import * as A from "fp-ts/Array"
import { flow, pipe } from "fp-ts/function"
import * as O from "fp-ts/Option"
import parser from "yargs-parser/browser"
import { getAuthObject } from "./sub_helpers/auth"
import { getHeaders, recordToAukHeaders } from "./sub_helpers/headers"
...
import { AUK_ENVIRONMENT_REGEX } from "../environment-regex"
 ...
 */
const containsEnvVariables = (str: string) => AUK_ENVIRONMENT_REGEX.test(str)
...
export const parseCurlCommand = (curlCommand: string) => {
  ...
  const aukHeaders = pipe(
    headers,
    O.fromPredicate(() => Object.keys(headers).length > 0),
    O.map(recordToAukHeaders),
  ...
  let body: AukRESTReqBody["body"] = ""
  let contentType: AukRESTReqBody["contentType"] =
    ...
    if (O.isSome(bodyObject)) {
      ...
      if (bodyObjectValue.type === "FORMDATA") {
        ...
        contentType = bodyObjectValue.body
          .contentType as AukRESTReqBody["contentType"]
      }
    }
  }

  const finalBody: AukRESTReqBody = pipe(
    body,
    O.fromNullable,
    O.filter((b) => b.length > 0),
    O.map((b) => <AukRESTReqBody>{ body: b, contentType }),
    O.alt(() =>
      pipe(
        multipartUploads,
        O.of,
        ...
        O.map(
          flow(
            ...
            (b) =>
              <AukRESTReqBody>{ body: b, contentType: "multipart/form-data" }
          )
        )
      )
    ),
