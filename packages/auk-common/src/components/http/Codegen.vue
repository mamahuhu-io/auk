...
import { AggregateEnvironment, getAggregateEnvs } from "~/store/environments"
...
import { getCurrentEnvironment } from "../../store/environments"
...
const resolveRequestAuthAndHeaders = () => {
  ...
  const resolvedAuth: AukRESTAuth =
    auth.authType === "inherit" && auth.authActive
      ? ((inheritedProperties?.auth?.inheritedAuth as AukRESTAuth) ?? {
          authType: "none",
          authActive: false,
        })
      : auth
  ...
  const resolvedHeaders: AukRESTHeaders = [...inheritedHeaders, ...headers]
 ...
 */
const buildFinalRequest = (auth: AukRESTAuth, headers: AukRESTHeaders) => {
  return {
