 ...
 * @param param Data of auk-collection with auk-requests, envs to be processed.
...
const processCollection = async (
  ...
  for (const request of collection.requests) {
    const _request = preProcessRequest(request as AukRESTRequest, collection);
  ...
  for (const folder of collection.folders) {
    const updatedFolder: AukCollection = { ...folder };
 ...
 * @param collections Auk-collection objects to be mapped to collection-stack type.
 * @returns Mapped collections to collection-stack.
 */
const getCollectionQueue = (collections: AukCollection[]): CollectionQueue[] =>
  pipe(
    collections,
    A.map(
...
/**
 * Exiting auk cli process with appropriate exit code depending on
