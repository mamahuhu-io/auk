...
const getAukCollVariables = (ig: ItemGroup<Item>): AukCollectionVariable[] => {
...
const getAukResponseBody = (body: string | ArrayBuffer | undefined): string => {
  if (!body) return ""
    ...
    pipe(
      ...
      A.map((response) => {
        const res = {
          ...
          body: getAukResponseBody(response.body),
          headers: getAukReqHeaders(response.headers),
          code: response.code,
          originalRequest: makeAukRESTResponseOriginalRequest({
            auth: getAukReqAuth(response.originalRequest?.auth),
            body: getAukReqBody({
              body: response.originalRequest?.body,
              headers: response.originalRequest?.headers ?? null,
            }) ?? { contentType: null, body: null },
            endpoint: getAukReqURL(response.originalRequest?.url ?? null),
            headers: getAukReqHeaders(
            ...
            params: getAukReqParams(
              response.originalRequest?.url.query ?? null
            ),
            requestVariables: getAukReqVariables(
...
const getAukReqAuth = (
  aukAuth: Item["request"]["auth"] | null
): AukRESTAuth => {
  if (!aukAuth) return { authType: "inherit", authActive: true }

  const auth = aukAuth as unknown as PMRequestAuthDef
    ...
    return pipe(
      ...
      O.bind("contentType", () =>
        pipe(
          ...
          getAukReqHeaders(headers),
          A.findFirst(({ key }) => key.toLowerCase() === "content-type"),
          O.map((x) => x.value),

          // Make sure it is a content-type Auk can work with
          O.filter(
...
const getAukReqURL = (url: Item["request"]["url"] | null): string => {
  if (!url) return ""
  return pipe(
    url.toString(false),
    S.replace(/\?.+/g, ""),
...
const getAukRequest = (item: Item, importScripts: boolean): AukRESTRequest => {
  const { preRequestScript, testScript } = getAukScripts(item, importScripts)
  return makeRESTRequest({
    name: item.name,
    endpoint: getAukReqURL(item.request.url),
    method: item.request.method.toUpperCase(),
    headers: getAukReqHeaders(item.request.headers),
    params: getAukReqParams(item.request.url.query),
    auth: getAukReqAuth(item.request.auth),
    body: getAukReqBody({
      body: item.request.body,
      headers: item.request.headers,
    }),
    requestVariables: getAukReqVariables(item.request.url.variables),
    responses: getAukResponses(item.responses),
    preRequestScript,
    testScript,
...
const getAukFolder = (
  ...
  makeCollection({
    ...
    folders: pipe(
      ...
      A.map((folder) => getAukFolder(folder, importScripts))
    ),
    requests: pipe(
      ig.items.all(),
      A.filter(isPMItem),
      A.map((item) => getAukRequest(item, importScripts))
    ),
    auth: getAukReqAuth(ig.auth),
    headers: [],
    variables: getAukCollVariables(ig),
    description: getCollectionDescription(ig.description),
  })

export const getAukCollections = (
  collections: PMCollection[],
  importScripts: boolean
) => {
  return collections.map((collection) =>
    getAukFolder(collection, importScripts)
  )
}

export const aukPostmanImporter = (
  fileContents: string[],
  importScripts = false
) =>
  pipe(
    ...
    O.map((collections) => {
      ...
      if (importScripts && fileContents.length > 0) {
        ...
        if (!isSupported) {
          ...
          return getAukCollections(collections, false)
        }
      }

      return getAukCollections(collections, importScripts)
    }),
