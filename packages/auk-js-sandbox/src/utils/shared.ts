 ...
 * Experimental sandbox version - Returns methods organized by namespace (`pw` and `auk`)
 */
...
export function getSharedEnvMethods(
  ...
  // Methods exclusive to the `auk` namespace
  const envResetFn = (
    key: string,
    options: EnvAPIOptions = { source: "all" }
  ) => {
  ...
  // Experimental scripting sandbox (Both `pw` and `auk` namespaces)
  if (isAukNamespace) {
    return {
      methods: {
        pw: {
