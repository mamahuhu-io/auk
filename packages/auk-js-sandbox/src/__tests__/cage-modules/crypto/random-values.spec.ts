...
describe("crypto.getRandomValues()", () => {
  test("should generate random values for array", () => {
    const script = `
      ...
      auk.test("Random values generated", () => {
        auk.expect(result.length).toBe(10)
        auk.expect(hasNonZero).toBe(true)
        auk.expect(result).toBe(array) // Should mutate in place
      })
    `
  ...
  test("should generate different values on multiple calls", () => {
    const script = `
      ...
      auk.test("Random values are different", () => {
  ...
  test("should handle different array sizes", () => {
    const script = `
      ...
      auk.test("Handles various array sizes", () => {
        for (const result of results) {
          auk.expect(result.length).toBe(result.size)
          auk.expect(result.hasRandomValues).toBe(true)
        }
      })
    `
  ...
  test("should return values in valid byte range (0-255)", () => {
    const script = `
      ...
      auk.test("Values are valid bytes", () => {
        auk.expect(allInRange).toBe(true)
        auk.expect(hasVariety).toBe(true)
      })
    `
...
describe("crypto.randomUUID()", () => {
  test("should generate valid UUID v4 format", () => {
    const script = `
      ...
      auk.test("UUID format is valid", () => {
        auk.expect(typeof uuid).toBe("string")
        auk.expect(uuid.length).toBe(36)
        auk.expect(uuidPattern.test(uuid)).toBe(true)
      })
    `
  ...
  test("should generate unique UUIDs", () => {
    const script = `
      ...
      auk.test("UUIDs are unique", () => {
        auk.expect(uniqueUuids.size).toBe(100)
      })
    `
  ...
  test("should generate UUIDs with correct version and variant", () => {
    const script = `
      ...
      auk.test("UUID version and variant are correct", () => {
        auk.expect(version).toBe('4')
        auk.expect(['8', '9', 'a', 'b'].includes(variant)).toBe(true)
      })
    `
