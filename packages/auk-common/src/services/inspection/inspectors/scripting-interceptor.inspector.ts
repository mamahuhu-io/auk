 ...
 * 1. Using Proxy interceptors with fetch/auk.fetch/pm.sendRequest
 *    - Proxy behavior is unknown
  ...
  implements Inspector
{
  ...
  /**
   * Detects if script contains fetch(), auk.fetch(), or pm.sendRequest() calls.
  ...
  private scriptContainsFetchAPI(script: string): string | null {
    ...
    } else if (/auk\.fetch\s*\(/i.test(script)) {
      return "auk.fetch()"
    } else if (/(?<!auk\.)fetch\s*\(/i.test(script)) {
      return "fetch()"
    }
  ...
  getInspections(
    req: Readonly<Ref<AukRESTRequest | AukRESTResponseOriginalRequest>>,
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _res: Readonly<Ref<AukRESTResponse | null | undefined>>
  ): Ref<InspectorResult[]> {
    return computed(() => {
      ...
      const request = req.value as AukRESTRequest
      ...
      // Warning 1: Proxy interceptor doesn't support scripting API calls
      if (currentInterceptorId === "proxy") {
        results.push({
