...
import { URL } from "url";
import { EffectiveAukRESTRequest, RequestConfig } from "../interfaces/request";
 ...
 */
const processEnvs = (envs: Partial<AukEnvs>) => {
 ...
 */
export const createRequest = (req: EffectiveAukRESTRequest): RequestConfig => {
 ...
 * Else, AukCLIError with appropriate error code & data.
 */
export const requestRunner =
  (
    requestConfig: RequestConfig
  ): TE.TaskEither<AukCLIError, RequestRunnerResponse> =>
  async () => {
    const start = hrtime();
...
export const processRequest =
  ...
  ): T.Task<{ envs: AukEnvs; report: RequestReport }> =>
  async () => {
    ...
    let effectiveRequest = <EffectiveAukRESTRequest>{
      ...request,
    ...
    const requestRunnerRes = await delayPromiseFunction<
      E.Either<AukCLIError, RequestRunnerResponse>
 ...
 * @param request Auk rest request to be processed.
