...
import { AukSpotlightSessionEventData } from "~/platform/analytics"
import { PersistenceService } from "~/services/persistence"
import { SpotlightService } from "~/services/spotlight"
import { UIExtensionService } from "~/services/ui-extension.service"
import GitConflictBanner from "~/components/workspace/GitConflictBanner.vue"
import GitConflictModal from "~/components/workspace/GitConflictModal.vue"
...
let unlistenMenuOpenSettings: (() => void) | null = null
...
onMounted(async () => {
      ...
      action: [
        {
          ...
          onClick: async (_, toastObject) => {
            ...
            window
              .open("https://auk.mamahuhu.dev/support/privacy", "_blank")
              ?.focus()
          },
        },
        {
  ...
  unlistenMenuOpenSettings = await listenDesktopEvent(
    "menu-open-settings",
    () => {
...
defineActionHandler("modals.search.toggle", (_, trigger) => {
  const triggerMethodMap: Record<
    InvocationTriggers,
    AukSpotlightSessionEventData["method"]
  > = {
