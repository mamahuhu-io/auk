...
import { AukRESTAuth } from "../../rest/v/13/auth"
...
export const V8_SCHEMA = v8_baseCollectionSchema.extend({
  folders: z.lazy(() => z.array(entityRefUptoVersion(AukCollection, 8))),
  ...
  up(old: z.infer<typeof V7_SCHEMA>) {
    const result: z.infer<typeof V8_SCHEMA> = {
      ...
      folders: old.folders.map((folder) => {
        const result = AukCollection.safeParseUpToVersion(folder, 8)
