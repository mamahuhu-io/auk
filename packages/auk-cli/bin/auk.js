...
if (!semver.satisfies(currentNodeVersion, requiredNodeVersionRange)) {
  console.error(
    `${chalk.greenBright("Auk CLI")} requires Node.js ${highlightVersion(requiredNodeVersion)} or higher and you're on Node.js ${highlightVersion(currentNodeVersion)}.`
  );

  console.error(
    `\nIf you prefer staying on Node.js ${highlightVersion("20")}, you can install the last supported version of the CLI:\n` +
      `${chalk.green(`npm install -g @auk/cli@${lastSupportedVersion}`)} alongside ${highlightVersion("2025.10.1")} of the Auk app.\n`
  );
  process.exit(1);
}
