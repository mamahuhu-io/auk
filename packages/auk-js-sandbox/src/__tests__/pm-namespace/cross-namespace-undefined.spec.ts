...
describe("Cross-namespace undefined preservation", () => {
  test("auk.env.get can read undefined set by pm.environment.set", () => {
    return expect(
      runTest(
        `
          pm.environment.set("undef_var", undefined)
          const value = auk.env.get("undef_var")
  ...
  test("auk.env.active.get can read undefined set by pm.variables.set", () => {
    return expect(
      runTest(
        `
          pm.variables.set("var_undef", undefined)
          const value = auk.env.active.get("var_undef")
          pm.expect(value).toBe(undefined)
        `,
        {
          global: [],
  ...
  test("auk.env.global.get can read undefined set by pm.globals.set", () => {
    return expect(
      runTest(
        `
          pm.globals.set("global_test", undefined)
          const value = auk.env.global.get("global_test")
          pm.expect(value).toBe(undefined)
        `,
        {
          global: [],
    ...
    return expect(
      runTest(
        `
          ...
          // Read from auk namespace
          const aukValue = auk.env.get("multi_read")
          pm.expect(aukValue).toBe(undefined)
    ...
    return expect(
      runTest(
        `
          ...
          pm.expect(auk.env.get("changeable")).toBe(undefined)

          // Overwrite with string via PM
          pm.environment.set("changeable", "new_value")
          pm.expect(auk.env.get("changeable")).toBe("new_value")
        `,
        {
          global: [],
          selected: [],
