...
pub fn perform_version_check_and_backup<R: Runtime>(app: AppHandle<R>) -> Result<(), AukError> {
...
fn get_current_app_version<R: Runtime>(app: &AppHandle<R>) -> Result<Version, AukError> {
...
fn backup_exists_for_version(version: &Version) -> Result<bool, AukError> {
...
fn backup_current_data(current_version: &Version) -> Result<(), AukError> {
...
fn copy_directory_recursive(src: &PathBuf, dst: &PathBuf) -> Result<(), AukError> {
    if !src.exists() {
        return Ok(());
    }
...
fn cleanup_old_backups() -> Result<(), AukError> {
    let backup_dir = path::backup_dir()?;
