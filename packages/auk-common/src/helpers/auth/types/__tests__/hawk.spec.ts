...
import { AukRESTAuth } from "@auk/data"

// Mock the calculateHawkHeader function
vi.mock("@auk/data", async () => {
  const actual = await vi.importActual("@auk/data")
  return {
    ...actual,
...
const { calculateHawkHeader } = await import("@auk/data")
...
describe("Hawk Auth", () => {
  ...
  describe("generateHawkAuthHeaders", () => {
    test("generates hawk authorization header", async () => {
      ...
      const auth: AukRESTAuth & { authType: "hawk" } = {
        authActive: true,
        authType: "hawk",
        authId: "<<HAWK_ID>>",
        authKey: "<<HAWK_KEY>>",
    ...
    test("returns empty array for non-hawk auth type", async () => {
      const auth: AukRESTAuth & { authType: "basic" } = {
        authActive: true,
        authType: "basic",
        username: "user",
        password: "pass",
