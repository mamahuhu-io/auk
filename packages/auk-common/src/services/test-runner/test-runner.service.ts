...
import { AukTestData, AukTestResult } from "~/helpers/types/AukTestResult"
import { AukTab } from "../tab"
...
export type TestRunnerRequest = AukRESTRequest & {
  type: "test-response"
...
export class TestRunnerService extends Service {
  ...
  public runTests(
    tab: Ref<AukTab<AukTestRunnerDocument>>,
  ...
  private async runTestCollection(
    ...
    options: TestRunnerOptions,
    parentPath: number[] = [],
    parentHeaders?: AukRESTHeaders,
    parentAuth?: AukRESTRequest["auth"],
    parentVariables: AukCollection["variables"] = [],
    parentID?: string
  ) {
    try {
  ...
  private getTestResultInfo(testResult: AukTestData) {
    let passed = 0
    let failed = 0
