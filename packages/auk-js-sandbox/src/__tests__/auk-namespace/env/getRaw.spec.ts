  ...
  test("returns the value defined in selected if also present in global", () => {
    return expect(
      runTest(
        `
          const data = auk.env.getRaw("a")
          auk.expect(data).toBe("selected val")
        `,
        {
          global: [
            {
...
describe("auk.env.active.getRaw", () => {
  test("returns only from selected", () => {
    return expect(
      runTest(
        `
          auk.expect(auk.env.active.getRaw("a")).toBe("a-selected")
          auk.expect(auk.env.active.getRaw("b")).toBe(null)
        `,
        {
          selected: [
            {
  ...
  test("returns null if key absent in selected", () => {
    return expect(
      runTest(
        `
          auk.expect(auk.env.active.getRaw("missing")).toBe(null)
        `,
        {
          selected: [],
          global: [
...
describe("auk.env.global.getRaw", () => {
  test("returns only from global", () => {
    return expect(
      runTest(
        `
          auk.expect(auk.env.global.getRaw("b")).toBe("b-global")
          auk.expect(auk.env.global.getRaw("a")).toBe(null)
        `,
        {
          selected: [
            {
  ...
  test("returns null if key absent in global", () => {
    return expect(
      runTest(
        `
          auk.expect(auk.env.global.getRaw("missing")).toBe(null)
        `,
        {
          selected: [
            {
