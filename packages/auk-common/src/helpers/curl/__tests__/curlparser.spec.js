...
import { makeRESTRequest, rawKeyValueEntriesToString } from "@auk/data"
import { parseCurlToAukRESTReq } from ".."

const samples = [
  {
    command: `
      curl --request GET \
      --url https://echo.mamahuhu.dev/ \
  ...
  {
    command: `curl --get -I -d "tool=curl" -d "platform=auk" -d"io" https://auk.mamahuhu.io`,
    response: makeRESTRequest({
      method: "HEAD",
      name: "Untitled",
      endpoint: "https://auk.mamahuhu.io/?io",
      auth: {
        authActive: true,
        authType: "inherit",
      },
  ...
  {
    command: `curl 'https://auk.mamahuhu.io/' \
    -H 'authority: auk.mamahuhu.io' \
    ...
    response: makeRESTRequest({
      method: "GET",
      name: "Untitled",
      endpoint: "https://auk.mamahuhu.io/",
      auth: { authType: "inherit", authActive: true },
      body: {
        contentType: null,
        body: null,
  ...
  },
  {
    ...
    --url 'https://echo.mamahuhu.dev/?hello=there' \
    --header 'content-type: application/x-www-form-urlencoded' \
  ...
  {
    command: "curl echo.mamahuhu.dev -A pinephone",
    response: makeRESTRequest({
      name: "Untitled",
      endpoint: "https://echo.mamahuhu.dev/",
      method: "GET",
  ...
  {
    command: "curl echo.mamahuhu.dev -G",
    response: makeRESTRequest({
      name: "Untitled",
      endpoint: "https://echo.mamahuhu.dev/",
      method: "GET",
  ...
  {
    command: `curl --get -I -d "tool=auk" https://example.org`,
    response: makeRESTRequest({
      name: "Untitled",
...
describe("Parse curl command to Auk REST Request", () => {
  for (const [i, { command, response }] of samples.entries()) {
    test(`for sample #${i + 1}:\n\n${command}`, () => {
      const actual = parseCurlToAukRESTReq(command)
