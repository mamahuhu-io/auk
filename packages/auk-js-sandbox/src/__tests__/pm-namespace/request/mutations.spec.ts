import { AukRESTAuth, AukRESTRequest } from "@auk/data"
  ...
  test("pm.request.url.protocol mutation changes protocol from https to http", () => {
    return expect(
      runPreRequestScript(
        ...
        console.log("auk.request.url:", auk.request.url)
        `,
  ...
  test("pm.request.method mutation changes HTTP method", () => {
    return expect(
      runPreRequestScript(
        ...
        console.log("auk.request.method:", auk.request.method)
        `,
        { envs, request: baseRequest }
      )
    ).resolves.toEqualRight(
      expect.objectContaining({
        ...
        consoleEntries: expect.arrayContaining([
          ...
          expect.objectContaining({ args: ["auk.request.method:", "POST"] }),
        ]),
      })
    )
  })
  ...
  test("pm.request.body.update() changes request body with Postman format", () => {
    ...
    return expect(
      runPreRequestScript(
        ...
        console.log("auk.request.body:", auk.request.body)
        `,
  ...
  test("pm.request.auth setter changes authentication", () => {
    ...
    return expect(
      runPreRequestScript(
        ...
        console.log("auk.request.auth:", auk.request.auth)
        `,
        { envs, request: baseRequest }
      )
    ).resolves.toEqualRight(
